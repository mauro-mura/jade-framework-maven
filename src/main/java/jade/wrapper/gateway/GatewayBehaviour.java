...
/**
 * This is a cyclic behaviour that processes the commands received via
 * JadeGateway. <code>JadeGateway</code> enables two alternative ways to
 * implement a gateway that allows non-JADE code to communicate with JADE
 * agents. <br>
 * The first one is to extend the <code>GatewayAgent</code> (see its javadoc for
 * reference). <br>
...
public abstract class GatewayBehaviour extends CyclicBehaviour {
	...
	/**
	 * Queue of all pending commands that have not yet been released (see method
	 * releaseCommand) In this Hash, the key is the Object while the value is the
	 * Event
	 */
	private final HashMap<Object, Event> commandQueue = new HashMap<>(2);
	...
	/**
	 * subclasses must implement this method. The method is called each time a
	 ...
	 * <code>releaseCommand</code>. Sometimes, you might prefer launching a new
