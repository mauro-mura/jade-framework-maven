...
/**
 * A profile implementation enhanced to support boot's argument parsing. This
 * class serves as the bridge between boot properties and profile properties. It
 * defines a collection of property keys which correspond to boot argument
 * names. These are used to access the boot properties from the argument
 * properties. The class Profile defines a similar collection of keys which are
 * used to access profile properties.
 * 
...
public class BootProfileImpl extends ProfileImpl {
	...
	/**
	 * Return the properties collection which resulted from the arguments. This
	 * collection is used to create/modify the underlying profile's properties.
	 * 
	...
	/**
	 * Copy a collection of argument properties into the existing argument
	 * properties and then into the profile properties. When moving between the
	 * argument properties and profile properties different keys are required.
	 * 
	 * @param source A collection of argument properties. The keys to this
	 *               collection are from the XXX_KEY strings defined in this class.
	 */
	public void setArgProperties(ExtendedProperties source) {
		...
		if (isMain) {
			...
			// Since the value is false, we cancel the default done in ProfileImpl's
			// constructor
		...
		/*
		 * PER IL DF value = argProp.getProperty(df.KBDF_MAX_RESULTS); if (value !=
		 * null) { profileProp.setProperty(df.KBDF_MAX_RESULTS, value); } value =
		 * argProp.getProperty(df.DBDF_DRIVER); if (value != null) {
		 * profileProp.setProperty(df.DBDF_DRIVER, value); } value =
		 * argProp.getProperty(df.DBDF_URL); if (value != null) {
		 * profileProp.setProperty(df.DBDF_URL, value); }
		 */

		/*
		 * value = argProp.getProperty(PASSWD_KEY); if (value != null) {
		...
		if (value != null) {
			try {
				Vector<Specifier> v = Specifier.parseSpecifierList(value);
				// Convert the Vector into a List
				List<Specifier> l = new ArrayList<>(v.size());
				Enumeration<Specifier> e = v.elements();
		...
		if (value != null) {
			Vector<String> agentVector = helper.T2(value, false);
			List<Specifier> agents = new ArrayList<>();

			for (Enumeration<Specifier> e = helper.getCommandLineAgentSpecifiers(agentVector); e.hasMoreElements();) {
				agents.add(e.nextElement());
			}
		...
		// "dumpProfile" attribute.
		// Note All the java.util.ArrayList structures will only print their type
		// unless a
	 ...
	 * @return True or false depending on the attributes setting. False if attribute
	 *         doesn't exist.
	 * @throws PropertiesException if there is a value but its not either "true" or
	 *                             "false".
	 */
	...
	/**
	 * Parse a String reading for a set of <code>parameter(arg)</code> each
	 * delimited by a <code>;</code> and no space in between.
	 * <p>
	 * For instance <code>jade.mtp.iiop(50);http.mtp.http(8080)</code> is a valid
	 * string, while <code>jade.mtp.iiop(50 80);http.mtp.http(8080)</code> is not
	 * valid For each object specifier, a new java object <code>Specifier</code> is
	 * added to the passed <code>out</code> List parameter.
	 */
