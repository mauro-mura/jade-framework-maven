...
/**
 * This class show a List of incoming and outgoing ACLmessages. When clicking on
 * the right button a pop menu wil be shown, containing operations on the
 * selected ACLMessage
 *
...
//logging
	...
	public void addMessageNode(String direction, String theTime, ACLMessage theACL) {
		...
		if (direction.startsWith("i")) {
			this.inMsgCount++;
			aclString = theACL.getPerformative(theACL.getPerformative()) + ": from  " + theSender + " - "
					+ contentString + " ";
		} else {
			this.outMsgCount++;
			aclString = theACL.getPerformative(theACL.getPerformative()) + ": to  " + theReceiver + " - "
					+ contentString + " ";
		...
		int i = 0;
		aclModel.insertNodeInto(new DefaultMutableTreeNode(":act " + theACL.getPerformative(theACL.getPerformative())),
				messageNode, i++);
		...
		AID aid = new AID();
		Iterator<AID> it = theACL.getAllReceiver();
		while (it.hasNext()) {
			aid = it.next();
		...
		// reply-to
		Iterator<AID> replyItor = theACL.getAllReplyTo();
	...
	private void jbInit() throws Exception {
		...
		this.add(aclTreeScrollPane, new GridBagConstraints(0, 1, 5, 1, 1.0, 1.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		this.add(jLabel1, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		this.add(sortComboBox, new GridBagConstraints(1, 0, 1, 1, 1.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
		this.add(ascRadioButton, new GridBagConstraints(2, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		this.add(descRadioButton, new GridBagConstraints(3, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		this.add(sortButton, new GridBagConstraints(4, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
...
//  ***EOF***
	...
	private class TraceFileFilter extends javax.swing.filechooser.FileFilter {
		...
		public boolean accept(File pathName) {
			if (pathName.isDirectory()) {
				return true;
			} else if (pathName.isFile() && (pathName.getName().endsWith(".trc"))) {
				return true;
			} else {
				return false;
			}
	...
	private class AboutFrame extends JWindow {
		...
		private void jbInit() throws Exception {
			...
			contentPanel.add(jLabel1, new GridBagConstraints(0, 3, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
					GridBagConstraints.NONE, new Insets(0, 0, 5, 0), 0, 0));
			contentPanel.add(jLabel2, new GridBagConstraints(0, 4, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
					GridBagConstraints.NONE, new Insets(10, 0, 10, 0), 0, 0));
			contentPanel.add(logoLabel3, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
					GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
			contentPanel.add(jLabel4, new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
...
//  ***EOF***
	...
	private class ACLTreeRenderer extends JLabel implements TreeCellRenderer {
		...
		/**
		 * Gets the TreeCellRendererComponent attribute of the ACLTreeRenderer object
		 *
	...
	private class SortRenderer extends JLabel implements ListCellRenderer {
		...
		/**
		 * Gets the ListCellRendererComponent attribute of the ACLPerformativesRenderer
		 * object
		 *
