 ...
 * <a href="../../../../intro.htm" target="_top">Introduction to the
 * Sniffer</a>.
 * <p>
 ...
 *         mouse over)
 * @author Moreno LAGO
 * @version $Date: 2010-06-11 15:32:31 +0200 (ven, 11 giu 2010) $ $Revision:
 *          6352 $
 *
 */
public class Sniffer extends ToolAgent {
		...
		public void action() {
			...
			if (current != null) {
				...
				try {
					...
					/*
					 * If this is a 'posted-message' event and the sender is // currently under
					 * sniff, then the message was already // displayed when the 'sent-message'
					 * event occurred. In that // case, we simply skip this message. if(ev
					 * instanceof PostedMessage) { Agent a = new Agent(msg.getSender());
	 ...
	 * 
	 * @param aMatchExpression An expression string with special significance to '?'
	 *                         and '*'.
	...
	/**
	 * ACLMessages for subscription and unsubscription as <em>rma</em> are created
	 * and corresponding behaviours are set up.
	 */
	public void toolSetup() {
		...
		/*
		 * #DOTNET_INCLUDE_BEGIN //Create GUI System.Threading.ThreadStart tStart = new
		 * System.Threading.ThreadStart( createUI ); System.Threading.Thread t = new
		 * System.Threading.Thread( tStart ); t.Start(); #DOTNET_INCLUDE_END
		 */
	}
	 ...
	 * preloaded. Also supports message filtering based on performatives. Each line
	 * of the file lists an agent, optionally followed by a set of messages to
	 * sniff. If there are no tokens, then we assume that means to sniff all. The
	 * tokens are the name of the performative type such as INFORM, QUERY, etc.
	 * 
	 ...
	 * <em>Sniffer</em> agent is destroyed, disconnecting from <em>AMS</em> agent
	 * and closing down the Sniffer administration <em>GUI</em>. Currently sniffed
	 * agents are also unsniffed to avoid errors.
	 */
	...
	/**
	 * This method adds an AMSClientBehaviour that performs a request to the AMS for
	 * sniffing/unsniffing list of agents.
	 **/
	 ...
	 *                  <li>Sniffer.SNIFF_OFF to deactivate sniffer on an
	 *                  agent/group
	 *                  </ul>
	 */
	...
	/**
	 * Inner class RequestListenerBehaviour. This behaviour serves requests to start
	 * sniffing agents. If an agent does not exist it is put into the preload table
	 * so that it will be sniffed as soon as it starts.
	 */
	private class RequestListenerBehaviour extends SimpleAchieveREResponder {
		...
		RequestListenerBehaviour() {
			...
			super(Sniffer.this, MessageTemplate.and(MessageTemplate.MatchPerformative(ACLMessage.REQUEST),
