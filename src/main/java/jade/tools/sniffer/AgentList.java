...
/**
 * The List for the agents on the Agent Canvas. Implements Serializable for
 * saving data to the binary snapshot file.
 */
...
public class AgentList implements Serializable {
	...
	public void removeAgent(String agentName) {
		...
		while (it.hasNext()) {
			Agent agent = it.next();
			if (agentName.equals(agent.agentName) && agent.onCanv) {
				agents.remove(agent);
			}
		}
	}
	...
	public boolean isPresent(String agName) {
		...
		while (it.hasNext()) {
			Agent agent = it.next();
			if (agent.agentName.equals(agName)) {
				return true;
			}
		}
		return false;
