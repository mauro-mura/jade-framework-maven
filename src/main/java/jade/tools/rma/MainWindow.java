 ...
 * @author Moreno LAGO
 * @version $Date: 2010-04-12 18:07:05 +0200 (lun, 12 apr 2010) $ $Revision:
 *          6302 $
 */
class MainWindow extends JFrame {
	...
	public MainWindow(rma anRMA) {
		...
		JMenuItem temp = popupRemote.add((RMAAction) actPro.actions.get(ActionProcessor.CUSTOM_ACTION));
		temp.setIcon(null);
		temp = popupRemote.add((RMAAction) actPro.actions.get(ActionProcessor.REGISTERREMOTEAGENTWITHAMS_ACTION));
		temp.setIcon(null);
		...
		JMenuItem tmp = popLocalPlatform.add((RMAAction) actPro.actions.get(ActionProcessor.VIEWPLATFORM_ACTION));
		tmp.setIcon(null);
		popLocalPlatform.add((RMAAction) actPro.actions.get(ActionProcessor.MANAGE_MTPS_ACTION));
		...
		getContentPane().add(new ToolBar(tree, this, actPro), "North"); // new ToolBar(tree, this,
	...
	public void addAgent(final String containerName, final AID agentID, final String state, final String ownership) {
		...
		Runnable addIt = new Runnable() {
			public void run() {
				...
				Iterator<String> add = agentID.getAllAddresses();
	...
	public boolean showUninstallMTPDialog(jade.domain.JADEAgentManagement.UninstallMTP umtp) {
		...
		String address = (String) JOptionPane.showInputDialog(this, "Choose the MTP to remove.", "Remove an MTP",
