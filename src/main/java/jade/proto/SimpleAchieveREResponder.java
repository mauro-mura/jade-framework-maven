 ...
 * achieved or not. Note that this is a simple implementation of the
 * <code>AchieveREResponder</code>. that does not allow to register Behaviour
 * for the Prepare Response and Prepare Result Notification states of the
 * protocol
 *
 ...
 * @author Moreno LAGO
 * @version $Date: 2013-05-21 11:40:42 +0200 (mar, 21 mag 2013) $ $Revision:
 *          6673 $
 **/
...
public class SimpleAchieveREResponder extends SimpleBehaviour implements FIPANames.InteractionProtocol {
	 ...
	 * the interaction protocol and the performative) into the constructor of this
	 * behaviour.
	 * 
	 ...
	 * @param mt    is the MessageTemplate that must be used to match the initiator
	 *              message. Take care that if mt is null every message is consumed
	 *              by this protocol.
	...
	/**
	 * This method is called when the initiator's message is received that matches
	 * the message template passed in the constructor. This default implementation
	 * return null which has the effect of sending no reponse. Programmers should
	 ...
	 *         use the method createReply of the class ACLMessage in order to create
	 *         a good reply message
	...
	/**
	 * This method is called after the response has been sent and only when one of
	 * the folliwing two cases arise: the response was an <code>agree</code> message
	 * OR no response message was sent. This default implementation return null
	 * which has the effect of sending no result notification. Programmers should
	 * override the method in case they need to react to this event.
	 * 
	 * @param request  the received message
	 ...
	 *         createReply of the class ACLMessage in order to create a good reply
	 *         message
	 * @see jade.lang.acl.ACLMessage#createReply()
	...
	/**
	 * This method allows to change the <code>MessageTemplate</code> that defines
	 * what messages this FIPARequestResponder will react to and reset the protocol.
	 */
	 ...
	 * 
	 * @return <code>true</code> if this behaviour has completed its task,
