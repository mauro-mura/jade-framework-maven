...
/**
 * This is simple implementation of the AchieveREInitiator. This implementation
 * in particular is 1:1 and does not allow the possibility to add Handler.
 * 
 ...
 * FIPA-subscribe, that allows the initiator to verify if the expected rational
 * effect of a single communicative act has been achieved.
 * <p>
 ...
 * @author Moreno LAGO
 * @version $Date: 2005-09-16 15:54:46 +0200 (ven, 16 set 2005) $ $Revision:
 *          5780 $
 **/
...
public class SimpleAchieveREInitiator extends SimpleBehaviour {
	 ...
	 *            that in this simple implementation, the
	 *            <code>prepareMessage</code> method returns a single message.
	...
	public final void action() {
		...
		case SEND_MSG_STATE: {
			// send the message. If there is more than one receiver only the first will be
			// taken into account.
			...
			if (request == null) {
				...
				if (receivers.hasNext()) {
					if (logger.isLoggable(Logger.WARNING)) {
						logger.log(Logger.WARNING,
							"The message you are sending has more than one receivers. The message will be sent only to the first one !!");
					}
				}
				...
				// set the timeout
				// FIXME: if the Timeout is already expired before the message will be sent, it
				// will be considered a infinite timeout
		...
		case RECEIVE_2ND_REPLY_STATE: {
			...
			if (secondReply != null) {
				...
				case ACLMessage.INFORM: {
					// call the method handleAllResponses since if an agree was arrived it was not
					// called.
					state = ALL_RESULT_NOTIFICATION_RECEIVED_STATE;
		...
		case ALL_REPLIES_RECEIVED_STATE: {
			...
			// call the handleAllResponses and then the handleAllResultNotification without
			// any message.
			state = ALL_RESULT_NOTIFICATION_RECEIVED_STATE;
			handleAllResponses((Vector) (getDataStore().get(ALL_RESPONSES_KEY)));
			break;
	 ...
	 * implementation just return the ACLMessage object passed in the constructor.
	 * Programmer might override the method in order to return a different
	 * ACLMessage. Note that for this simple version of protocol, the message will
	 * be just send to the first receiver set.
	 * 
	 ...
	 * received, which is not out-of-sequence according to the protocol rules. This
	 * default implementation does nothing; programmers might wish to override the
	 ...
	 * timeout is expired. By response message we intend here all the
	 * <code>agree, not-understood,
	...
	/**
	 * This method resets this behaviour so that it restarts from the initial state
	 * of the protocol with a null message.
	 */
	public void reset() {
		reset(null);
	}
