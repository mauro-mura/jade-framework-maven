...
public class AchieveREInitiator extends Initiator {
	 ...
	 * passed in the constructor of the class.
	 **/
	public final String REQUEST_KEY = INITIATION_K;
	/**
	 * key to retrieve from the DataStore of the behaviour the vector of ACLMessage
	 * objects that have been sent.
	 **/
	public final String ALL_REQUESTS_KEY = ALL_INITIATIONS_K;
	/**
	 * key to retrieve from the DataStore of the behaviour the last ACLMessage
	 * object that has been received (null if the timeout expired).
	 **/
	public final String REPLY_KEY = REPLY_K;
	/**
	 * key to retrieve from the DataStore of the behaviour the vector of ACLMessage
	 * objects that have been received as response.
	 **/
	public final String ALL_RESPONSES_KEY = "__all-responses" + hashCode();
	/**
	 * key to retrieve from the DataStore of the behaviour the vector of ACLMessage
	 * objects that have been received as result notifications.
	 **/
	 ...
	 * @param msg   The message that must be used to initiate the protocol. Notice
	 *              that the default implementation of the
	 *              <code>prepareRequest()</code> method returns an array composed
	 *              of only this message. The values of the slot
	 *              <code>reply-with</code> is ignored and a different value is
	 *              assigned automatically by this class for each receiver.
		...
		if (s != null) {
			...
			if (s.update(perf)) {
				...
				case Session.RESULT_NOTIFICATION_RECEIVED:
					...
					break;
				default:
					// Something went wrong. Return false --> we will go to the HANDLE_OUT_OF_SEQ
					// state
					return false;
				}
	...
	/**
	 * Check the status of the sessions after the reception of the last reply or the
	...
	protected String[] getToBeReset() {
		if (toBeReset == null) {
			toBeReset = new String[] { HANDLE_AGREE, HANDLE_REFUSE, HANDLE_NOT_UNDERSTOOD, HANDLE_INFORM,
					HANDLE_FAILURE, HANDLE_OUT_OF_SEQ };
		}
		return toBeReset;
	}
	// #APIDOC_EXCLUDE_END
	...
	/**
	 * This method must return the vector of ACLMessage objects to be sent. It is
	 * called in the first state of this protocol. This default implementation just
	 * returns the ACLMessage object passed in the constructor. Programmers might
	 * prefer to override this method in order to return a vector of objects for 1:N
	 * conversations or also to prepare the messages during the execution of the
	 * behaviour.
	 * 
	 * @param request the ACLMessage object passed in the constructor
	 * @return a Vector of ACLMessage objects. The values of the slot
	...
	/**
	 * This method is called every time an <code>agree</code> message is received,
	 * which is not out-of-sequence according to the protocol rules. This default
	 * implementation does nothing; programmers might wish to override the method in
	 * case they need to react to this event.
	 * 
	...
	/**
	 * This method is called every time a <code>refuse</code> message is received,
	 * which is not out-of-sequence according to the protocol rules. This default
	 * implementation does nothing; programmers might wish to override the method in
	 * case they need to react to this event.
	 * 
	...
	/**
	 * This method is called when all the responses have been collected or when the
	 * timeout is expired. The used timeout is the minimum value of the slot
	 * <code>replyBy</code> of all the sent messages. By response message we intend
	 * here all the <code>agree, not-understood,
	 * refuse</code> received messages, which are not not out-of-sequence according
	 * to the protocol rules. This default implementation does nothing; programmers
	 * might wish to override the method in case they need to react to this event by
	 * analysing all the messages in just one call.
	 * 
	 ...
	 * <code>inform, 
	 * failure</code> received messages, which are not not out-of-sequence according
	 * to the protocol rules. This default implementation does nothing; programmers
	 * might wish to override the method in case they need to react to this event by
	 * analysing all the messages in just one call.
	 * 
	 ...
	 * PREPARE_REQUESTS state. This behaviour would override the homonymous method.
	 ...
	 * registered behaviour to put the Vector of ACLMessage objects to be sent into
	 * the datastore at the <code>ALL_REQUESTS_KEY</code> key. The values of the
	 * slot <code>reply-with</code> is ignored and a different value is assigned
	 * automatically by this class for each receiver.
	 * 
	...
	/**
	 * This method allows to register a user defined <code>Behaviour</code> in the
	 * HANDLE_AGREE state. This behaviour would override the homonymous method. This
	 * method also set the data store of the registered <code>Behaviour</code> to
	 * the DataStore of this current behaviour. The registered behaviour can
	 * retrieve the <code>agree</code> ACLMessage object received from the datastore
	 * at the <code>REPLY_KEY</code> key.
	 * 
	 * @param b the Behaviour that will handle this state
	 */
	 ...
	 * This method also set the data store of the registered <code>Behaviour</code>
	 * to the DataStore of this current behaviour. The registered behaviour can
	 * retrieve the <code>inform</code> ACLMessage object received from the
	 * datastore at the <code>REPLY_KEY</code> key.
	 * 
	 * @param b the Behaviour that will handle this state
	 */
	...
	/**
	 * This method allows to register a user defined <code>Behaviour</code> in the
	 * HANDLE_REFUSE state. This behaviour would override the homonymous method.
	 * This method also set the data store of the registered <code>Behaviour</code>
	 * to the DataStore of this current behaviour. The registered behaviour can
	 * retrieve the <code>refuse</code> ACLMessage object received from the
	 * datastore at the <code>REPLY_KEY</code> key.
	 * 
	 * @param b the Behaviour that will handle this state
	 */
	...
	/**
	 * This method allows to register a user defined <code>Behaviour</code> in the
	 * HANDLE_ALL_RESPONSES state. This behaviour would override the homonymous
	 * method. This method also set the data store of the registered
	 * <code>Behaviour</code> to the DataStore of this current behaviour. The
	 * registered behaviour can retrieve the vector of ACLMessage objects, received
	 * as a response, from the datastore at the <code>ALL_RESPONSES_KEY</code> key.
	 * 
	 * @param b the Behaviour that will handle this state
	 */
	...
	/**
	 * This method allows to register a user defined <code>Behaviour</code> in the
	 * HANDLE_ALL_RESULT_NOTIFICATIONS state. This behaviour would override the
	 * homonymous method. This method also set the data store of the registered
	 * <code>Behaviour</code> to the DataStore of this current behaviour. The
	 * registered behaviour can retrieve the Vector of ACLMessage objects, received
	 * as a result notification, from the datastore at the
	 * <code>ALL_RESULT_NOTIFICATIONS_KEY</code> key.
	 * 
	 * @param b the Behaviour that will handle this state
	 */
	...
	private static class Session implements ProtocolSession, Serializable {
		...
		/**
		 * return true if the received performative is valid with respect to the current
		 * session state.
		 */
