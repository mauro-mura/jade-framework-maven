...
class Stub implements java.io.Serializable {
	...
	protected List<TransportAddress> remoteTAs = new ArrayList<>();

	// The ID of the remote object
	protected int remoteID;

	// The name of the platform this node belongs to (only used when the
	// enablemultipleplatform option is set)
	...
	/**
	 * Build a stub for a given already remotized object. Note that TAs are added
	 * separately as we don't know at this stage whether this is a Stub to reach a
	 * remote object or a Stub for a local object.
	 */
	...
	/**
	 * Add a TransportAddress to the list of addresses of the remote object this is
	 * a stub of
	 */
	...
	protected int checkResult(Command result, String[] expectedExceptions) throws IMTPException {
		if (result.getCode() == Command.ERROR) {
			...
			throw new IMTPException(
					exceptionName + " occurred in remote container [" + (String) result.getParamAt(1) + "]");
