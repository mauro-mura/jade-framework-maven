...
/**
 * The <code>ServiceManagerStub</code> class is the remote proxy of JADE
 * platform <i>Service Manager</i> and <i>Service Finder</i> components, running
 * over LEAP transport layer.
 * 
...
class PlatformManagerStub extends Stub implements PlatformManager {
	...
	public String addNode(NodeDescriptor desc, List<ServiceDescriptor> services, boolean propagated)
		...
		try {
			...
			cmd.addParam(Boolean.valueOf(propagated));
	...
	public void removeNode(NodeDescriptor desc, boolean propagate) throws IMTPException, ServiceException {
		try {
			...
			cmd.addParam(desc);
			cmd.addParam(Boolean.valueOf(propagate));
