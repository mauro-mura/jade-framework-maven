 ...
 * serialization mechanism. The LEAP surrogate serialization mechanism only
 * throws this type of exception and encapsulates an occurred exception. This
 * way, all possible exceptions during LEAP serialization can be handled by
 * handling this <code>LEAPSerializationException</code>. If the detailed
 * exception is needed, it is available in field <code>detail</code>.
 * 
...
public class LEAPSerializationException extends Exception {
	...
	/**
	 * Constructs a <code>LEAPSerializationException</code> with no specified detail
	 * message.
	 */
	...
	/**
	 * Get the detail message, including the message from the nested exception if
	 * there is one.
	 * 
