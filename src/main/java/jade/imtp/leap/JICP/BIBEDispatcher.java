...
//		}
		...
		/*
		 * lastSid try { lastSid = (byte) (Integer.parseInt(props.getProperty("outcnt"))
		 * -1); if (lastSid < 0) { lastSid = 0x0f; } } catch (Exception e) { // Keep
		 * default }
		 */
	...
	/**
	 * Shutdown self initiated or forced by the JICPServer this BackEndContainer is
	 * attached to.
	 */
	public void kill() {
	...
	/**
	 * This is called by the JICPServer when a JICP packet addressing this mediator
	 * as recipient-ID is received. In the case of the BIBEDispatcher this should
	 * never happen.
	 */
	...
	public boolean handleIncomingConnection(Connection c, JICPPacket pkt, InetAddress addr, int port) {
		...
		if (data.length == 1) {
			...
			try {
				inp = "inp".equals(new String(data));
			} catch (Exception e) {
			}
		}
		if (inp) {
			...
			if ((currentTime - lastReceivedTime) > (keepAliveTime + 60000)) {
				...
				if (inpHolder.isConnected()) {
					Thread t = new Thread() {
						public void run() {
							try {
								JICPPacket pkt = new JICPPacket(JICPProtocol.KEEP_ALIVE_TYPE, JICPProtocol.DEFAULT_INFO,
										null);
	...
	private synchronized JICPPacket dispatchPacket(JICPPacket pkt, boolean flush, int oldSessionId)
			throws ICPException {
	...
	/**
	 * Inner class InpConnectionHolder. Wrapper for the connection used to deliver
	...
	/**
	 * Inner class OutConnectionHolder Wrapper for the connection used to receive
