...

{
	...
	public JICPServer(Profile p, JICPPeer myPeer, ICP.Listener l, ConnectionFactory f, int max) throws ICPException {
		...
		if (host == null || Profile.LOCALHOST_CONSTANT.equals(host)) {
			...
			if (remoteURL != null) {
				// Retrieve the local host address by means of the GET_ADDRESS JICP
				// functionality
			...
			// Unless the UNCKECK_LOCAL_HOST property is set, if a local-host is explicitly
			// specified check
			// that it is a valid local address
			if (!p.getBooleanProperty(UNCHECK_LOCAL_HOST, false) && !Profile.isLocalHost(host)) {
				throw new ICPException("Error: Not possible to launch JADE on a remote host (" + host
		...
		boolean changePortIfBusy = !p.getBooleanProperty(Profile.MAIN, true)
				|| p.getBooleanProperty(LEAPIMTPManager.CHANGE_PORT_IF_BUSY, false);
		...
		if (strPortRange != null) {
			try {
				...
				myLogger.log(Logger.WARNING,
						"Wrong local-port-range format " + strPortRange + ". Must be <min>-<max>. " + e);
			}
		}
		...
		if (portRange == null) {
			...
			if (server == null) {
				...
				throw new ICPException("Cannot bind server socket to " + (host != null ? "host " + host : "localhost")
						+ " and any ports in range " + strPortRange);
			}
		}
	...
	public String getLocalHost() {
		// If a local-host was not specified, we accept connection on all local network
		// addresses,
	...
	/**
	 * JICPServer thread entry point. Accept incoming connections and for each of
	 * them start a ConnectionHandler that handles it.
	 */
	public void run() {
		int chCount = 0;
		...
		synchronized (connectionHandlers) {
			...
			Enumeration<ConnectionHandler> en = connectionHandlers.elements();
		...
		// Close all mediators
		Enumeration<JICPMediator> e = mediators.elements();
		while (e.hasMoreElements()) {
			JICPMediator m = e.nextElement();
			m.kill();
		}
		mediators.clear();
	...
	/**
	 * Inner class ConnectionHandler. Handle a connection accepted by this
	 * JICPServer
	 */
				...
				do {
						...
						if (recipientID != null) {
							...
							if (type == JICPProtocol.COMMAND_TYPE) {
								...
								if (connectionHandlers.size() >= maxHandlers) {
									// Too many connections open --> close the connection as soon as the command has
									// been served
					...
					case JICPProtocol.CREATE_MEDIATOR_TYPE:
						if (acceptMediators) {
							...
							// If the platform-name is specified refuse the request: JICPServer does not
							// accept this kind of mediator creation request
							String pn = p.getProperty(Profile.PLATFORM_ID);
							if (pn != null) {
								myLogger.log(Logger.WARNING,
										"CREATE_MEDIATOR request with specified platform-name: " + pn);
								reply = new JICPPacket(JICPProtocol.NOT_AUTHORIZED_ERROR,
										new JADESecurityException("Platform-name specified"));
								break;
							}
							...
							if (myPDPContextManager != null) {
								try {
									// FIXME: Get username and password specified among the CREATE_MEDIATOR
									// properties
									Properties pdpContextInfo = myPDPContextManager.getPDPContextInfo(addr,
											p.getProperty(Profile.OWNER));
							...
							myLogger.log(Logger.WARNING,
									"CREATE_MEDIATOR request received with accept-mediator option set to false. Address is: "
											+ addr);
					...
					case JICPProtocol.CONNECT_MEDIATOR_TYPE:
						if (acceptMediators) {
							...
							myLogger.log(Logger.WARNING,
									"CONNECT_MEDIATOR request received with accept-mediator option set to false. Address is: "
											+ addr);
							reply = new JICPPacket(JICPProtocol.NOT_AUTHORIZED_ERROR, null);
						}
						break;
