...
import jade.imtp.leap.exception.ICPDispatchException;
...
public class FrontEndDispatcher implements FEConnectionManager, Dispatcher, TimerListener, Runnable {
	...
	// Host and port to bind the local side of the connection with the BE (useful in
	// case more than one Network Interface is available)
	...
	public BackEnd getBackEnd(FrontEnd fe, Properties props) throws IMTPException {
		...
		try {
			...
			// Response timeout offset (actual timeout is offset + increment proportional to
			// packet length)
			...
			// Set active to true before creating the back-end. This is because in this
			// phase
			// we may receive a SYNCH command and the CommandServer must be there to process
			// it
			active = true;
	...
	/*
	 * private void manageRemoteConfig() throws IMTPException { String
	 * remoteConfigHost =
	 * myProperties.getProperty(MicroRuntime.REMOTE_CONFIG_HOST_KEY); String
	 * remoteConfigPort =
	 * myProperties.getProperty(MicroRuntime.REMOTE_CONFIG_PORT_KEY); if
	 * (remoteConfigHost != null && remoteConfigPort != null) { // Remote
	 * configuration options specified: Retrieve connectivity related //
	 * configurations from the indicated host and port JICPConnection c =
	 * getConnection(new JICPAddress(remoteConfigHost, remoteConfigPort,
	 * myMediatorID, "")); JICPPacket pkt = new
	 * JICPPacket(JICPProtocol.GET_CONFIG_OPTIONS_TYPE, JICPProtocol.DEFAULT_INFO,
	 * null); writePacket(pkt, c); pkt = c.readPacket(); c.close(); ... String
	 * replyMsg = new String(pkt.getData()); if (pkt.getType() !=
	 * JICPProtocol.ERROR_TYPE) { // BackEnd creation successful
	...
	/**
	 * Send the CREATE_MEDIATOR command with the necessary parameter in order to
	 * create the BackEnd in the fixed network. Executed - at bootstrap time by the
	 * thread that creates the FrontEndContainer. - To re-attach to the platform
	 * after a fault of the BackEnd
	 */
	private JICPConnection createBackEnd() throws IMTPException {
		StringBuilder sb = BackEndStub.encodeCreateMediatorRequest(myProperties);
		...
		JICPPacket pkt = new JICPPacket(JICPProtocol.CREATE_MEDIATOR_TYPE, JICPProtocol.DEFAULT_INFO, null,
				sb.toString().getBytes());

		// Try first with the current transport address, then with the various backup
		// addresses
	...
	private JICPConnection create(JICPPacket createPkt) throws IOException, ICPException {
		...
		if (createRsp.getType() != JICPProtocol.ERROR_TYPE) {
			...
			// JICP Error
			if (myConnectionListener != null && replyMsg != null
					&& replyMsg.startsWith(JICPProtocol.NOT_AUTHORIZED_ERROR)) {
	...
	private String[] parseBackEndAddresses(String addressesText) {
		Vector<String> addrs = Specifier.parseList(addressesText, ';');
		// Convert the list into an array of strings
		String[] result = new String[addrs.size()];
		for (int i = 0; i < result.length; i++) {
			result[i] = addrs.elementAt(i);
		}
		return result;
	}
	...
	private void initServerTimeOffset(Connection con) {
		...
		try {
			...
			if (rsp.getType() != JICPProtocol.ERROR_TYPE) {
				...
				myLogger.log(Logger.INFO, "Server time initialized: value=" + serverTime + ", offset=" + offset
			...
			if (terminator != myCommandServer) {
				...
				try {
					if (connectionDropped) {
						...
						terminationPacket = new JICPPacket(JICPProtocol.CONNECT_MEDIATOR_TYPE,
								JICPProtocol.TERMINATED_INFO, mediatorTA.getFile(), null);
					} else {
						terminationPacket = new JICPPacket(JICPProtocol.COMMAND_TYPE, JICPProtocol.TERMINATED_INFO,
								null);
					}
	...
	public synchronized byte[] dispatch(byte[] payload, boolean flush, int oldSessionId) throws ICPException {
		...
		if (myConnection != null) {
			...
			try {
				...
				myLogger.log(getLevel(Logger.FINEST),
						myMediatorID + " - Waiting for response, SID = " + pkt.getSessionID());
				...
				if (response != null) {
					...
					if (myLogger.isLoggable(getLevel(Logger.FINER))) {
						myLogger.log(getLevel(Logger.FINER),
								myMediatorID + " - Response received " + response.getSessionID());
					}
					...
					myLogger.log(Logger.WARNING,
							myMediatorID + " - Response timeout expired. SID = " + pkt.getSessionID());
					handleDisconnection();
					throw new ICPDispatchException("Response timeout expired [2]", pkt.getSessionID());
				}
			} catch (IOException ioe) {
				// Can't reach the BackEnd.
				myLogger.log(Logger.WARNING, myMediatorID + " - Error writing command. SID = " + pkt.getSessionID(),
						ioe);
				handleDisconnection();
			...
			if (!refreshingConnection) {
				// The previous attempt to reconnect failed. Activate the reconnection mechanism
				// again
				handleDisconnection();
			}
	 ...
	 * packets (incoming commands and responses to outgoing commands)
	 */
	private class ConnectionReader extends Thread {
		...
		public void run() {
			...
			try {
				while (isConnected()) {
					...
					myLogger.log(getLevel(Logger.FINER),
							myMediatorID + " - CR-" + myId + " packet received, SID=" + pkt.getSessionID());
				...
				synchronized (connectionLock) {
					if (active && !connectionDropped) {
						...
						// This synchronized check avoids that an old connection reader suddenly
						// realizes that its connection is down
						// and tries to refresh an already restored connection
						if (this == FrontEndDispatcher.this.myConnectionReader) {
							// Unblock any Thread waiting for a response. It will behave as if the response
							// timeout was expired
		...
		private JICPPacket handleIncomingPacket(JICPPacket pkt) {
			...
			case JICPProtocol.COMMAND_TYPE:
				myLogger.log(getLevel(Logger.FINE),
						myMediatorID + " - CR-" + myId + " COMMAND received from BE, SID=" + pkt.getSessionID());
				updateTimers();
				serveCommand(pkt);
				myLogger.log(getLevel(Logger.FINEST),
						myMediatorID + " - CR-" + myId + " Incoming command passed to asynchronous command server");
				break;
			...
			case JICPProtocol.ERROR_TYPE:
				myLogger.log(getLevel(Logger.FINER),
						myMediatorID + " - CR-" + myId + " RESPONSE/ERROR received from BE. " + pkt.getSessionID());
	...
	// This is synchronized to be sure that commands and responses are always
	// written in a non-overlapping way
	private synchronized void writePacket(JICPPacket pkt, Connection c) throws IOException {
		...
		/*
		 * else { updateKeepAlive(); if (pkt.getType() != JICPProtocol.KEEP_ALIVE_TYPE
		 * && pkt.getType() != JICPProtocol.DROP_DOWN_TYPE) {
		...
		synchronized (responseLock) {
			try {
				while (lastOutgoingResponse == null) {
					...
					if (lastOutgoingResponse != null
							&& (sessionID != -1 && lastOutgoingResponse.getSessionID() != sessionID)) {
						myLogger.log(Logger.WARNING,
								"Wrong sessionID in response from BE: type=" + lastOutgoingResponse.getType() + " info="
										+ lastOutgoingResponse.getInfo() + " SID=" + lastOutgoingResponse.getSessionID()
										+ " while " + sessionID + " was expected.");
						// Go back waiting
						lastOutgoingResponse = null;
						continue;
					}
	...
	public void run() {
		...
		int cnt = 0;
		// This is used to switch between short and large interval between successive
		...
		while (active) {
			try {
				...
				// Unexpected error. This may be due to strange situations where for instance a
				// spurious KA response is interpreted as a CONNECT_MEDIATOR response
	...
	private void connect(int attemptCnt) throws IOException, IMTPException, ICPException {
		myLogger.log(Logger.INFO, myMediatorID + " - Connecting to " + mediatorTA.getHost() + ":" + mediatorTA.getPort()
				+ " " + attemptCnt);
		Connection c = openConnection(mediatorTA);
		myLogger.log(Logger.INFO, myMediatorID + " - Connection opened");
		JICPPacket pkt = new JICPPacket(JICPProtocol.CONNECT_MEDIATOR_TYPE, JICPProtocol.DEFAULT_INFO,
				mediatorTA.getFile(), null);
		writePacket(pkt, c);
	...
	/**
	 * Clear the keep-alive and drop-down timers. Mutual exclusion with
	 * updateKeepAlive(), updateConnectionDropDown() and doTimeOut()
	 */
	...
	/**
	 * Refresh the keep-alive timer. Mutual exclusion with clearTimers(),
	 * updateConnectionDropDown() and doTimeOut()
	 */
	...
	/**
	 * Refresh the connection drop-down timer. Mutual exclusion with clearTimers(),
	 * updateKeepAlive() and doTimeOut()
	 */
	...
	/**
	 * Mutual exclusion with updateKeepAlive(), updateConnectionDropDown() and
	 * clearTimers()
	 */
	...
	/**
	 * Send a KEEP_ALIVE packet to the BE. Mutual exclusion with dispatch() and
	 * dropDownConnection().
	 */
	...
	/**
	 * Send a DROP_DOWN packet to the BE and close the connection. Mutual exclusion
	 * with dispatch() and sendKeepAlive().
	 */
	private synchronized void dropDownConnection(Timer t) {
		if (t != cdTimer) {
			...
			// - Data sent 1 sec after t expiration --> Connection dropped, but immediately
			// restored --> NOT dropped
			// In such case the check works properly thanks to the fact that
			// dropDownConnection()
			// and dispatch() are executed in mutual exclusion.
			// On the contrary, for data sent by the BE to the FE the check may not work
			// since
			// the ConnectionReader is not (and must not be) synchronized. In that case
			// however
			// the check is not important since:
			// - Data received 1 sec before t expiration --> Connection NOT dropped
			// - Data issued by BE 1 sec after t expiration --> Connection dropped (data
			// cannot be received)
			return;
		}
		if (myConnection != null && !connectionDropped) {
	...
	/**
	 * Inner class IncomingCommandServer Serving incoming commands asynchronously is
	 * necessary to support commands whose serving process involves issuing one or
	 * more outgoing commands. If such commands were served directly by the
	 * ConnectionReader thread, in facts, there would be no chance to get the
		...
		public void run() {
			...
			while (active) {
				...
				if (sid == lastSid) {
					...
					myLogger.log(Logger.WARNING, myMediatorID + " - Duplicated command from BE: info="
							+ currentCommand.getInfo() + ", SID=" + sid);
				} else {
	...
	/*
	 * #J2ME_INCLUDE_BEGIN private int getLevel(int level) { if (verbosity > 0) { if
	 * (level == Logger.FINE) { return Logger.INFO; } else if (level ==
	 * Logger.FINER) { return verbosity >= 2 ? Logger.INFO : Logger.FINE; } else if
	 * (level == Logger.FINEST) { return verbosity >= 3 ? Logger.INFO : verbosity ==
	 * 2 ? Logger.FINE : Logger.FINER; } } return level; } #J2ME_INCLUDE_END
	 */
}
