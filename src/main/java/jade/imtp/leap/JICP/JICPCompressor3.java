...
public class JICPCompressor3 {
	...
	/**
	 * run length encoding write if value is RLE_FLUSH, the stream will be flushed.
	 */
...
//        System.out.println("" + (count++) + ": " + (char)rleOldValue + " " + rleOldValue);
	...
	private int getValue(int index) {
		return index >= ba.length || index < 0 ? RLE_FLUSH : ((int) (char) ba[index]) & 255;
	}
...
//            System.out.println("[r]");
	 ...
	 * algorithm:
	 *
	 * if during compression a word is detected, its position inside the array is
	 * stored in the wordIndexes[] array. but only, if the word itself was not found
	 * in the array before. if so, only a magic byte (WORD_MAGIC=255) and the index
	 * of the word in the wordIndexes[] array is stored as a byte. the magic byte is
...
//        System.out.println("" + ba.length + "->" + result.length + " = " + (result.length*100)/ba.length+"%");
	 ...
	 * (WORD_MAGIC=255) is detected, it will be decoded to a "real" 255 if the
	 * following byte is also 255 (255, 255 - sequence). if the following byte is
	 * not 255, it is treated as an index in the wordIndexes[] array. the index
	 * stored inside there is used as the beginning of a word. this word will be
	 * copied to the end of the decoded stream. the end of the word is detected by a
	 * separator (see isSep()).
	 */
