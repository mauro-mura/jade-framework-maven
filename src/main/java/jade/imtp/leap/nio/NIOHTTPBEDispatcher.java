...
public class NIOHTTPBEDispatcher implements NIOMediator, Dispatcher, BEConnectionManager {
	...
	public void init(JICPMediatorManager mgr, String id, Properties props) throws ICPException {
		...
		// Counter to assign the SID to the next command to be delivered to the FE (only
		// present if this is a back-end re-creation)
		nextOutgoingCommandSid = 0;
		try {
			// The FE indicates the SID of the last command it received --> Increment it by
			// 1
		...
		// SID of last command received from the FE (only present if this is a back-end
		// re-creation)
		lastIncomingCommandSid = 0x10;
		try {
			// The FE indicates the SID of the next command it will send us --> Decrement it
			// by 1
	...
	/**
	 * Termination self initiated or forced by the MediatorManager we are attached
	 * to.
	 */
	public void kill() {
		status = NOT_ACTIVE;
		// Force the BackEndContainer to terminate.
		// This will also cause this NIOHTTPBEDispatcher to terminate and deregister
	...
	/**
	 * Handle an incoming connection. This is called by the MediatorManager when a
	 * CREATE or CONNECT_MEDIATOR request is received. In both cases the Front-end
	 * is connecting --> Set the front-end status to CONNECTING, but don't use this
	 * connection: to allow us sending commands to the front-end an initial dummy
	 * response will be received soon.
	 */
		...
		if ((status == ACTIVE) && (frontEndStatus != TERMINATED)) {
			if (myLogger.isLoggable(Logger.FINE)) {
				myLogger.log(Logger.FINE,
						myID + " - Incoming packet. Type = " + pkt.getType() + ", SID = " + pkt.getSessionID()
								+ ", terminated-info = " + ((pkt.getInfo() & JICPProtocol.TERMINATED_INFO) != 0));
			}
			...
			if (pkt.getType() == JICPProtocol.COMMAND_TYPE) {
				...
				if ((pkt.getInfo() & JICPProtocol.TERMINATED_INFO) != 0) {
					...
					// Since we return null the MediatorManager would keep the connection open -->
					// close it explicitly
			...
			myLogger.log(Logger.FINE,
					"Unexpected packet received after termination. Type = " + pkt.getType() + ", SID = "
							+ pkt.getSessionID() + ", terminated-info = "
							+ ((pkt.getInfo() & JICPProtocol.TERMINATED_INFO) != 0));
		}
	...
	/**
	 * Termination initiated by the BackEndContainer (i.e. by the platform). When
	 * the BackEndContainer exits
	 */
	...
	//////////////////////////////////////////
	/**
	 * This is called by the Stub using this Dispatcher to dispatch a serialized
	 * command to the FrontEnd. Mutual exclusion with itself to ensure one command
	 * at a time is dispatched.
	 */
	public synchronized byte[] dispatch(byte[] payload, boolean flush, int oldSessionId) throws ICPException {
		if (status == ACTIVE) {
			if (frontEndStatus == CONNECTED) {
				...
				// front-end has just reconnected but flushing of postponed commands has not
				// started yet
				...
				try {
					...
					JICPPacket response = getResponse(
							RESPONSE_TIMEOUT + RESPONSE_TIMEOUT_INCREMENT * (cmd.getLength() / 1024));
					if (myLogger.isLoggable(Logger.FINE)) {
					...
					throw new ICPDispatchException("Error delivering outgoing command to front-end. ",
							cmd.getSessionID());
				} catch (ICPException icpe) {
					// Note that in this case setFrontEndDisconnected() is already called within
					// getResponse() or getOutgoingCommandsConnection()
				...
				try {
					...
					if (isKeepAliveResponse(response)) {
						...
						myLogger.log(Logger.WARNING,
								"Unexpected response received while waiting for Keep-alive response");
					}
				} catch (IOException ioe) {
	...
	private void handleResponse(Connection c, JICPPacket response, String from) {
		if (frontEndStatus != CONNECTED) {
			if (frontEndStatus == CONNECTING) {
				...
				// Unexpected response
				myLogger.log(Logger.WARNING, myID + " - Unexpected (likely out of time) response received.",
						new Exception("DUMMY!!!!"));
			}
	...
	// No need for synchronization as this is always executed within a synchronized
	// block
