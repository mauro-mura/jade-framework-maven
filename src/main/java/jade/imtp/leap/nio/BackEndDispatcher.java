...
public class BackEndDispatcher implements NIOMediator, BEConnectionManager, Dispatcher {
	...
	public void init(JICPMediatorManager mgr, String id, Properties props) throws ICPException {
		...
		try {
			responseTimeoutMultiplicativeFactor = Double
					.parseDouble(props.getProperty(JICPProtocol.RESPONSE_TIMEOUT_MULTIPLICATIVE_FACTOR_KEY));
		} catch (Exception e) {
			// Keep default
		}
		...
		/*
		 * lastSid int lastSid = 0x0f; try { lastSid = (byte)
		 * (Integer.parseInt(props.getProperty("outcnt")) -1); if (lastSid < 0) {
		 * lastSid = 0x0f; } } catch (Exception e) { // Keep default }
		 * myLogger.log(Logger.INFO,
		 * "Last command successfully received from FE had sessionID "+lastSid);
		 */
	 ...
	 * when a peer termination notification is received.
	 */
	public void kill() {
	 ...
	 * This is called by the JICPMediatorManager this Mediator is attached to as
	 * soon as the mediated entity (re)connects.
	 * 
	 * @param c    the connection to the mediated entity
	 * @param pkt  the packet that was sent by the mediated entity when opening this
	 *             connection
	...
	public boolean handleIncomingConnection(Connection c, JICPPacket pkt, InetAddress addr, int port) {
		if (pkt.getType() == JICPProtocol.CONNECT_MEDIATOR_TYPE) {
			...
			// Thread executing this method hangs trying to enter the synchronized block
			// below
		...
		// Lock the buffer of pending commands (if any) for flushing
		// (MicroStub.beginFlush())
		// before entering the synchronized block to avoid deadlock with a dispatching
		// Thread
		// that already locked the buffer of pending commands for dispatching
		// (MicroStub.beginDispatch()),
		...
		synchronized (this) {
			...
			if (peerActive) {
				...
				// Activate flushing (if needed) only when the connection has been fully
				// re-established
				if (flusher != null) {
					flusher.start();
				}
	...
	/**
	 * Notify this NIOMediator that an error occurred on one of the Connections it
	 * is using. This information is important since, unlike normal mediators, a
	 * NIOMediator typically does not read packets from connections on its own (the
	 * JICPMediatorManager does that in general).
	 */
	public void handleConnectionError(Connection c, Exception e) {
		// In case someone was waiting for a response notify it the response will never
		// arrive
		inpManager.notifyIncomingResponseReceived(null);

		// NOTE (09 May 2016): The following 5 lines were included in the sinchronized
		// block below (***).
		// We moved it here since we experimented that writing to the SocketChannel in
		// certain cases
		// may take several minutes due to NoRouteToHost problem.
		// In such cases when the KEEP ALIVE timeout expires this method is called but
		// hangs trying to enter
		// the synchronized block below (lock held by the Thread executing dispatch()).
		// So we first close the connection (SocketChannel write should exit with
		// exception) and then
		// enter the synchronized block. Let's see if this modification does not have
		// side effects.
		try {
			c.close();
		} catch (Exception e1) {
	...
	/**
	 * Passes to this mediator a JICPPacket received by the JICPMediatorManager this
	 * mediator is attached to. In a NIOMediator this should never be called.
	 */
	 ...
	 * <code>Connection</code> the incoming JICPPacket was received from. This
	 * information is important since, unlike normal mediators, a NIOMediator may
	 * not read packets from connections on its own (the JICPMediatorManager does
	 * that in general).
	 */
	...
	/**
	 * This is periodically called by the JICPMediatorManager and is used by this
	 * NIOMediator to evaluate the elapsed time without the need of a dedicated
	 * thread or timer.
	 */
			...
			if (!connectionDropped) {
				...
				if (checkMaxDisconnectionTime(currentTime)) {
					myLogger.log(Logger.WARNING,
							myID + ": Max disconnection time expired. FrontEnd is likely dead --> Close BackEnd");
					// Consider as if the FrontEnd has terminated spontaneously -->
					// Kill the above container (this will also kill this BackEndDispatcher).
					kill();
				}
			} else {
				// If the FE stays dropped for to much time (1h), very likely it dead --> kill
				// the above container
				if ((currentTime - dropTimeStamp) > 3600000) {
					myLogger.log(Logger.WARNING,
							myID + ": Max drop-down time expired. FrontEnd is likely dead --> Close BackEnd");
					kill();
				}
			}
		}
	 ...
	 * @param props Additional (implementation dependent) connection configuration
	 *              properties.
	...
	public synchronized byte[] dispatch(byte[] payload, boolean flush, int oldSessionId) throws ICPException {
		if (connectionDropped) {
			...
			if (elapsed > 2000) {
				myLogger.log(Logger.WARNING,
						myID + " - JICP command dispatching time over threshold: " + elapsed + " ms");
			}
			return pkt.getData();
		}
	}
	...
	protected void handleDropDown(Connection c, JICPPacket pkt, InetAddress addr, int port) {
		try {
			if (inpManager.isEmpty()) {
				...
				// Should never happen (the inpManager would not be empty), but, in case someone
				// was waiting for a
	...
	/**
	 * Move from the connectionDropped state to the Disconnected state. This may
	 * happen when - a packet must be dispatched to the FE. - an incoming connection
	 * is detected
	 */
	...
	/**
	 * Request the FE to refresh the connection. This default implementation does
	 * nothing. Subclasses may redefine this method to exploit some application
	 * specific out-of-band channel
	 */
	...
	/**
	 * Inner class InputManager. This class manages the delivery of commands to the
	 * FrontEnd
	 */
			...
			try {
				if (active && isConnected()) {
					...
					try {
						...
						if (myLogger.isLoggable(Logger.FINE)) {
							myLogger.log(Logger.FINE, "[Thread=" + Thread.currentThread().getName() + "] BE " + myID
									+ " - Sending command to FE " + pkt.getSessionID());
						}
						writePacket(myConnection, pkt);
						if (myLogger.isLoggable(Logger.FINE)) {
							myLogger.log(Logger.FINE, "[Thread=" + Thread.currentThread().getName() + "] BE " + myID
									+ " - Waiting for response from FE " + pkt.getSessionID());
						}

						long responseTimeout = JICPProtocol.computeTimeout(responseTimeoutOffset,
								responseTimeoutMultiplicativeFactor, pkt.getLength());
						pkt = waitForResponse(inpCnt, responseTimeout);
						if (pkt != null) {
							if (myLogger.isLoggable(Logger.FINE)) {
								myLogger.log(Logger.FINE, "[Thread=" + Thread.currentThread().getName() + "] BE " + myID
										+ " - Response received from FE " + pkt.getSessionID());
							}
							...
							myLogger.log(Logger.WARNING, "[Thread=" + Thread.currentThread().getName() + "] BE " + myID
						...
						myLogger.log(Logger.WARNING,
								"[Thread=" + Thread.currentThread().getName() + "] BE " + myID + ": " + ioe);
			...
			try {
				while (lastIncomingResponse == null) {
					...
					if (lastIncomingResponse != null && lastIncomingResponse.getSessionID() != sessionID) {
						myLogger.log(Logger.WARNING,
								myID + ": Duplicated response from FE: type=" + lastIncomingResponse.getType()
										+ " info=" + lastIncomingResponse.getInfo() + " SID="
										+ lastIncomingResponse.getSessionID());
						// Go back waiting
						lastIncomingResponse = null;
						continue;
					}
	...
	/**
	 * Inner class OutputManager This class manages the reception of commands and
	 * keep-alive packets from the FrontEnd.
	 */
	protected class OutputManager {
		...
		final JICPPacket handleCommand(JICPPacket cmd) throws ICPException {
			...
			if (sid == lastSid && lastResponse != null) {
				myLogger.log(Logger.WARNING, myID + ": Duplicated packet from FE: pkt-type=" + cmd.getType() + " info="
						+ cmd.getInfo() + " SID=" + sid);
				reply = lastResponse;
			} else {
				if (myLogger.isLoggable(Logger.FINE)) {
	...
	private final boolean checkTerminatedInfo(JICPPacket pkt) {
		if ((pkt.getInfo() & JICPProtocol.TERMINATED_INFO) != 0) {
			...
			if (type == JICPProtocol.COMMAND_TYPE || type == JICPProtocol.RESPONSE_TYPE
				...
				if (pkt.getType() == JICPProtocol.COMMAND_TYPE) {
					// Spontaneous FE termination. Unblock any Thread waiting for a response. It
					// will behave as if the response timeout was expired
					inpManager.notifyIncomingResponseReceived(null);
				}
			}
		}
