 ...
 * for data exchanged between the created BEContainers and their FEContainers in
 * an asynchronous way using java.nio <br>
 * <br>
 ...
 * reconnections) at the agent level. <br>
 * <br>
 * This service accepts the following configuration parameters:<br>
 * <code>jade_imtp_leap_nio_BEManagementService_servers</code>: list of of
 * IOEventServer ids separated by ';'<br>
 * Actually this service is a collection of IOEventServer objects. Each
 * IOEventServer opens and manages a server socket, accepts BackEnd
 * creation/connection requests and passes incoming data to BEDispatchers
 * independently from the others. If the above parameter is not specified a
 * single IOEventServer will be started and its ID will be
 * <code>jade_imtp_leap_nio_BEManagementService</code>. All other parameters are
 * related to a single IOEventServer and must be specified in the form<br>
 * serverid_parametername<br>
 * They are:
 * <ul>
 * <li><code>serverid_local-host</code>: Specifies the local network interface
 * for the server socket managed by this server (defaults to localhost).</li>
 * <li><code>serverid_local-port</code>: Specifies the local port for the server
 * socket managed by this server (defaults to 2099)</li>
 * <li><code>serverid_protocol</code>: Specifies the protocol used by this
 * server in the form of <code>jade.imtp.leap.JICP.ProtocolManager<code> 
class
</li>
<li>
<code>serverid_additional-services</code>: Specifies additional services that
 * have to be activated in each BackEnd Container started by the indicated
 * server</li>
 * <li><code>serverid_leap-property-file</code>: Specifies the leap-property
 * file to be used by this server.</li>
 * <li><ode>serverid_poolsize</code>: Specifies the number of threads used by
 * this server to manage IO events.</li>
 * <ul>
 * 
...
public class BEManagementService extends BaseService {
	...
	// since this is the only service dealing with NIO it is ok to set the size used
	// for enlarging buffers here
	private static int bufferIncreaseSize = 1024;
	/**
	 * this property can be used to set how many bytes will be used when nio buffer
	 * need to be increased
	 */
	public static final String BUFFERINCREASE = "bufferincrease";

	private static final String[] OWNED_COMMANDS = new String[] { INCOMING_CONNECTION };
	...
	private void handleAcceptOption(Profile p) throws ProfileException {
		// Manage the shortcut configuration option
		// jade_imtp_leap_nio_BEManagementService_accept
		...
		if (acceptOption != null) {
			...
			// socket;http(8080)
			// This means that we will accept plain sockets on the default port and HTTP on
			// port 8080
			...
			for (int i = 0; i < acceptedProtocols.size(); ++i) {
				...
				if (proto.equalsIgnoreCase(MicroRuntime.SOCKET_PROTOCOL)
					...
					myLogger.log(Logger.WARNING,
							"Unsupported protocol " + proto + ". Permitted values are socket, ssl, http and https!!!!");
				}
			}
	...
	/**
	 * This method is called by the JADE runtime just after this service has been
	 * installed. It activates the IO event servers specified in the Profile and the
	 * Ticker thread.
	 */
	public void boot(final Profile p) throws ServiceException {
		...
		// Active all servers after a while to allow the correct starting and
		// registration to the main-container
		Thread t = new Thread() {
			public void run() {
				long wait = 10000;
				try {
	...
	private void initializeSAM() {
		try {
			...
			if (sam != null) {
				...
				// Counter of packets whose processing required more than 1 sec and less than 10
				// sec
	...
	/**
	 * This is called by the JADE Runtime when this service is deinstalled. It stops
	 * the Ticker thread and all active IO event servers.
	 */
	...
	/**
	 * Inner class IOEventServer. This class asynchronously manages a server socket
	 * and all IO Events that happen on it and on all sockets opened through it. The
	 * BEManagementService is basically a collection of these servers.
	 */
	private class IOEventServer implements PDPContextManager, PDPContextManager.Listener, JICPMediatorManager {
		...
		public void init(String id, Profile p) {
			...
			// Protocol
			/*
			 * Actually we only need a ConnectionFactory at least the way it is build at the
			 * moment.
			 *
			 * Perhaps a NIOJICPServer should do the Selector handling, now that is
			 * controlled by instances of a inner class LoopManager. Than a NIOJICPPeer
			 * controls this server and provides the ConnectionFactory
			 *
			 * In that way we stay closer the regular blocking io handling, more work
			 * involved though.....
			 *
			 */
			...
			try {
				...
				if (protoManagerClass != null) {
					myLogger.log(Logger.WARNING, myLogPrefix + "Unable to load protocol-manager class "
							+ protoManagerClass + ", fallback to default " + NIOJICPPeer.class.getName() + "!");
				}
				pm = new NIOJICPPeer();
			}
			...
			try {
				...
				myLogger.log(Logger.INFO,
						myLogPrefix + "Applying properties from file " + fileName + " to all back-ends");
			} catch (Exception e) {
				myLogger.log(Logger.CONFIG,
						myLogPrefix + "Can't read LEAP property file " + fileName + ". Keep default. [" + e + "]");
			...
			if (pdpContextManagerClass != null) {
				try {
					...
					myLogger.log(Logger.WARNING,
							myLogPrefix + "Cannot load PDPContext manager " + pdpContextManagerClass, t);
		...
		public synchronized void activate() throws Throwable {
			...
			// Register the Selector of LoopManager 0 to the ServerSocketChannel to be
			// notified about ACCEPT operations.
			// NOTE that registrations to Socket Channels for READ operations must be done
			// as long as SocketChannel are created
			mySSChannel.register(loopers[0].getSelector(), SelectionKey.OP_ACCEPT);
			myLogger.log(Logger.INFO, myLogPrefix + "Ready to accept I/O events on address "
					+ myProtocol.buildAddress(host, String.valueOf(port), null, null));
		...
		void replaceLoopManager(int index, LoopManager newLoopManager) {
			...
			while (it.hasNext()) {
				...
				try {
					...
					myLogger.log(Logger.INFO,
							Thread.currentThread().getName() + "- New Selector " + newLoopManager.getSelector()
									+ " successfully registered to Channel " + channel + " with interest options = "
									+ interestOps);
				} catch (Exception e) {
					myLogger.log(Logger.SEVERE,
							Thread.currentThread().getName() + "- Error registering new Selector "
									+ newLoopManager.getSelector() + " to Channel " + channel
									+ " with interest options = " + interestOps + "[" + e + "]");
				}
			}
		...
		/**
		 * Get the LoopManager with the minimum number of registered keys.
		 */
		final LoopManager getLooper() throws NotFoundException {
			int minSize = 999999; // Big value;
			int index = -1;
			// Start from 1: LM-0 is dedicated to handle ACCEPT-OP on the
			// ServerSocketChannel
		...
		/**
		 * Serve an IO event conveied by a SelectionKey (a JICPPacket or an IO
		 * exception). This method is executed by one of the threads in the server
		 * Thread pool.
		 */
		public void servePacket(KeyManager mgr, JICPPacket pkt) {
			...
			try {
				...
				case JICPProtocol.GET_SERVER_TIME_TYPE: {
					...
					myLogger.log(Logger.INFO,
							myLogPrefix + "GET_SERVER_TIME request received from " + address + ":" + port);
				...
				case JICPProtocol.GET_CONFIG_OPTIONS_TYPE: {
					...
					myLogger.log(Logger.INFO,
							myLogPrefix + "GET_CONFIGURATION_OPTIONS request received from " + address + ":" + port);
				...
				case JICPProtocol.CREATE_MEDIATOR_TYPE: {
					...
					if (mediator == null) {
						...
						myLogger.log(Logger.INFO,
								myLogPrefix + "CREATE_MEDIATOR request received from " + address + ":" + port);

						// Lock the connection to be sure nothing is written back until we have sent the
						// response (see NIOJICPConnectionWrapper)
						connection.lock();
						...
						if (pn != null && !pn.equals(platformName)) {
							myLogger.log(Logger.WARNING, myLogPrefix
									+ "Security attack! CREATE_MEDIATOR request with wrong platform name: " + pn);
							reply = new JICPPacket(JICPProtocol.NOT_AUTHORIZED_ERROR,
									new JADESecurityException("Wrong platform-name"));
							break;
						}
						...
						try {
							...
							myLogger.log(Logger.WARNING,
									myLogPrefix + "Security error! CREATE_MEDIATOR request from non authorized client ["
											+ address + "]. " + jse.getMessage());
						...
						if (id != null) {
							if (msisdn != null && !msisdn.equals(id)) {
								...
								myLogger.log(Logger.WARNING, myLogPrefix
										+ "Security attack! CREATE_MEDIATOR request with mediator-id != MSISDN. Address is: "
										+ address);
								reply = new JICPPacket(JICPProtocol.NOT_AUTHORIZED_ERROR,
						...
						if ("true".equals(p.getProperty(JICPProtocol.GET_SERVER_TIME_KEY))) {
							// A GET_SERVER_TIME request will arrive just after mediator creation --> do not
							// unlock the connection (this will be done after serving the GET_SERVER_TIME
							// request)
							keepLock = true;
						...
						myLogger.log(Logger.WARNING, myLogPrefix
								+ "CREATE_MEDIATOR request received on a connection already linked to an existing mediator");
				...
				case JICPProtocol.CONNECT_MEDIATOR_TYPE: {
					...
					if (mediator == null) {
						myLogger.log(Logger.INFO, myLogPrefix + "CONNECT_MEDIATOR request received from " + address
								+ ":" + port + ". ID=" + recipientID);
						...
						/*
						 * if (myPDPContextManager != null) { Properties pdpContextInfo =
						 * myPDPContextManager.getPDPContextInfo(address, "OWNER???"); if
						 * (pdpContextInfo != null) { String msisdn =
						 * pdpContextInfo.getProperty(PDPContextManager.MSISDN); if (msisdn == null ||
						 * !msisdn.equals(recipientID)) { myLogger.log(Logger.WARNING,
						 * myLogPrefix+"Security attack! CONNECT_MEDIATOR request with mediator-id != MSISDN. Address is: "
						 * +address); reply = new JICPPacket(JICPProtocol.NOT_AUTHORIZED_ERROR, null);
						 * break; } } else { myLogger.log(Logger.WARNING,
						 * myLogPrefix+"Security attack! CONNECT_MEDIATOR request from non authorized address: "
						 * +address); reply = new JICPPacket(JICPProtocol.NOT_AUTHORIZED_ERROR, null);
						 * break; } }
						 */
						...
						myLogger.log(Logger.WARNING, myLogPrefix
								+ "CONNECT_MEDIATOR request received on a connection already linked to an existing mediator");
						reply = new JICPPacket("Unexpected packet type", null);
					}
					break;
				}
				default: {
					// Pass all other JICP packets (commands, responses, keep-alives ...) to the
					// proper mediator.
					...
					if (mediator != null) {
						if (myLogger.isLoggable(Logger.FINEST)) {
							myLogger.log(Logger.FINEST, myLogPrefix + "Passing packet of type " + type + " to mediator "
									+ mediator.getID());
						}
				...
				if (type == JICPProtocol.COMMAND_TYPE || type == JICPProtocol.CREATE_MEDIATOR_TYPE
						|| type == JICPProtocol.CONNECT_MEDIATOR_TYPE || type == JICPProtocol.GET_ADDRESS_TYPE) {
			...
			if (reply != null) {
				try {
					...
					incomingPacketServingErrorCounter++;
					myLogger.log(Logger.WARNING, myLogPrefix + stringify(mediator)
							+ "Communication error writing return packet to " + address + ":" + port + " [" + e + "]",
							e);
			...
			try {
				if (closeConnection) {
					try {
						// Close connection
						if (myLogger.isLoggable(Logger.FINEST)) {
							myLogger.log(Logger.FINEST, myLogPrefix + stringify(mediator) + "Closing connection with "
									+ address + ":" + port);
						}
						connection.close();
					} catch (IOException io) {
						myLogger.log(Logger.WARNING, myLogPrefix + stringify(mediator)
								+ "I/O error while closing connection with " + address + ":" + port);
						io.printStackTrace();
					}
				}
			} finally {
				// STEP 4) Unlock the connection if necessary
				// (be sure this is executed even if there is an unexpected error in the closure
				// step)
		...
		public void serveException(KeyManager mgr, Exception e) {
			...
			if (!connection.isClosed()) {
				...
				if (mediator != null) {
					...
					InetAddress address = s.getInetAddress();
					int port = s.getPort();
					myLogger.log(Logger.WARNING, myLogPrefix + "Exception reading incoming packet from " + address + ":"
							+ port + " [" + e + "]");
				}
			}
		...
		/**
		 * Called by a Mediator to notify that it is no longer active. This is often
		 * called within the tick() method. In this case directly removing the
		 * deregistering mediator from the mediators table would cause a
		 * ConcurrentModificationException --> We just add the deregistering mediator to
		 * a queue of mediators to be removed. The actual remotion will occur at the
		 * next tick in a synchronized way.
		 */
				...
				if (!lm.isStuck()) {
					if (lm.getReadElapsedTime(currentTime) > 60000) {
						...
						if (lmThread.isInterrupted()) {
							// This LoopManager was already interrupted once and did not recover --> There
							// is nothing we can do. Mark it as STUCK
							lm.setStuck();
							int runningLoopersCnt = countRunningLoopers();
							myLogger.log(Logger.WARNING,
									"LM-" + lm.myIndex + " did not recover after last interrupt --> Mark it as STUCK. "
											+ runningLoopersCnt + " LoopManagers still working properly");
							...
							myLogger.log(Logger.WARNING, "LM-" + lm.myIndex
									+ " appears to be stuck in handling incoming data from the network. Try to kill it! Thread stack trace is\n"
									+ sb.toString());
							lmThread.interrupt();
						}
					}
		...
		///////////////////////////////////////
		/**
		 * If no PDPContextManager is specified in the properties file the IOEventServer
		 * itself is used and the default behaviour is to issue an INCOMING_CONNECTION
		 * outgoing command
		 */
	...
	/**
	 * Inner class KeyManager Keep a SelectionKey together with the information
	 * associated to it such as the connection wrapping the key channel and the
	 * mediator using that connection (if any).
	 */
	private class KeyManager {
		...
		/**
		 * Read some data from the connection associated to the managed key and let the
		 * IOEventServer serve it
		 */
		public final void read() {
			...
			try {
				...
				if (myLogger.isLoggable(Logger.FINE)) {
					myLogger.log(Logger.FINE, "Incomplete JICPPacket (" + pie.getMessage() + ") from connection "
							+ connection + ". Wait for more data..");
				}
			} catch (Exception e) {
	...
	private class LoopManager implements Runnable {
		...
		public void join() throws InterruptedException {
			...
			if (myThread.isAlive()) {
				myLogger.log(Logger.WARNING,
						"Thread " + myThread.getName() + " did not terminate when requested to do so");
			}
		}
		...
		public void run() {
			...
			// This call is necessary if this is a replaced LoopManager. It has no effects
			// otherwise
			...
			while (state == ACTIVE_STATE) {
				...
				try {
					...
					// There seems to be a bug in java.nio
					// (http://www.limewire.org/techdocs/nio2.html).
					// Just retry.
					myLogger.log(Logger.WARNING,
							myServer.getLogPrefix() + "NullPointerException in select. Ignore and retry.");
					continue;
				} catch (Exception e) {
				...
				if (state == ACTIVE_STATE) {
					...
					if (myLogger.isLoggable(Logger.FINE)) {
						myLogger.log(Logger.FINE, prefix + " n=" + n + " selected-keys=" + keysSize
								+ (n != keysSize ? " This is very strange!!!!" : ""));
					}
					if (n > 0 && keysSize > 0) {
						// NOTE that the check on keys.size() is redundant and is there just to handle
						// the "select bug" (see handelSelectBug())
						selectBugCounter = 0;
						...
						while (it.hasNext()) {
							...
							if (key.isValid()) {
								if ((key.readyOps() & SelectionKey.OP_ACCEPT) != 0) {
									// This is an incoming connection. The channel must be the SerevrSocketChannel
									// server
									if (myLogger.isLoggable(Logger.FINER)) {
										myLogger.log(Logger.FINER,
												prefix + "------------------ ACCEPT_OP on key " + key);
									}
									...
									myLogger.log(Logger.WARNING,
											prefix + "SelectedKey " + key + " has unknown OPTIONS " + key.readyOps());
								}
							} else {
						...
						if (selectBugCounter > 100) {
							// This means that we are looping due to problems in the select() (known Linux
							// JVM bug)
							handleSelectBug();
						}
					}
		...
		/**
		 * JDK 1.6 in Linux has a BUG so that Selector.select() in certain cases do not
		 * block causing an infinite loop. The workaround is to recreate the Selector.
		 * Actually we recreate the whole LoopManager
		 */
		private synchronized void handleSelectBug() {
			// Mutual exclusion with register()
			myLogger.log(Logger.WARNING,
					Thread.currentThread().getName() + " SELECT BUG OCCURRED!!!! Replacing LoopManager");
		...
		private final void handleAcceptOp(SelectionKey key, String prefix) {
			try {
				...
				lm.register(sc);
				myLogger.log(Logger.INFO,
						prefix + "Incoming socket " + sc + " assigned to LM-" + lm.myIndex + " (" + lm.size() + ")");
			} catch (JADESecurityException jse) {
		...
		private final void handleReadOp(SelectionKey key, String prefix) throws ICPException {
			...
			try {
				...
				if (mgr == null) {
					...
					key.attach(mgr);
					myLogger.log(Logger.INFO,
							prefix + "Connection " + c + " created and associated to KeyManager " + mgr);
				}
				mgr.read();
			} finally {
		...
		private synchronized final void handlePendingChannels(String prefix) {
			if (pendingChannelPresent) {
				for (int i = 0; i < pendingChannels.size(); ++i) {
					...
					if (myLogger.isLoggable(Logger.FINE)) {
						myLogger.log(Logger.FINE, prefix + "Registering Selector " + mySelector + " on channel " + sc
								+ " for READ operations");
					}
					try {
						sc.register(mySelector, SelectionKey.OP_READ);
					} catch (Exception e) {
						myLogger.log(Logger.WARNING, prefix + "Error registering socket channel for asynchronous IO. ",
								e);
					}
				}
	...
	/**
	 * Inner class BEManagementHelperImpl. Implementation class for the
	 * BEManagementHelper.
	 */
	...
	/**
	 * Check that the address of the initiator of a new connection is not in the
	 * list of addresses considered as malicious.
	 */
