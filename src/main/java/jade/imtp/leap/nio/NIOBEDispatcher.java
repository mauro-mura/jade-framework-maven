...
/**
 * This class implements the BIFEDispatcher related BackEnd dispatcher managable
 * by an asynchronous JICPMediatorManager
 * 
...
public class NIOBEDispatcher implements NIOMediator, BEConnectionManager, Dispatcher {
	 ...
	 * was using. This information is important since, unlike normal mediators, a
	...
	public JICPPacket handleJICPPacket(Connection c, JICPPacket pkt, InetAddress addr, int port) throws ICPException {
		...
		if (type == JICPProtocol.COMMAND_TYPE) {
			 ...
			 * Asynch-reply else if (type == JICPProtocol.RESPONSE_TYPE || type ==
			 * JICPProtocol.ERROR_TYPE) { inpManager.handleResponse(c, pkt); return null; }
			 */ else {
	...
	protected class InputManager {
		...
		/**
		 * Dispatch a JICP command to the FE and get back a reply. This method must NOT
		 * be executed in mutual exclusion with setConnection() and resetConnection()
		 * since it performs a blocking read operation --> It can't just be declared
		 * synchronized.
		 */
		...
		/*
		 * Asynch-reply final synchronized void handleResponse(Connection c, JICPPacket
		 * reply) throws ICPException { checkConnection(c); currentReply = reply;
		 * notifyAll(); }
		 * 
		 * private synchronized JICPPacket waitForReply(long timeout) throws
		 * ICPException, IOException { try { if (currentReply == null) { wait(timeout);
		 * if (currentReply == null) { if (isConnected()) { if (connectionRefreshed) {
		 * throw new ICPException("Connection refreshed"); } else { throw new
		 * IOException("Response timeout expired"); } } else { throw new
		 * ICPException("Connection reset"); } } } JICPPacket tmp = currentReply;
		 * currentReply = null; return tmp; } catch (InterruptedException ie) { throw
		 * new ICPException("Interrupted"); } }
		 */
	} // END of inner class InputManager

	/**
	 * Inner class OutputManager This class manages the reception of commands and
	 * keep-alive packets from the FrontEnd. This class also manages the
	 * maxDisconnectionTime, i.e. the remote FrontEnd is considered dead if it
	 * cannot re-establish the OUT connection within the maxDisconnectionTime.
	 */
