...
public class LEAPIMTPManager implements IMTPManager {
	...
	/**
	 * Default constructor used to dynamically instantiate a new LEAPIMTPManager
	 * object
	 */
	...
	public void initialize(Profile p) throws IMTPException {
		...
		try {
			...
			List<Specifier> l = theProfile.getSpecifiers(ICPS);
			Iterator<Specifier> it = l.iterator();
			while (it.hasNext()) {
				Specifier s = it.next();
				try {
		...
		List<String> URLs = theDispatcher.getLocalURLs();

		if (URLs.isEmpty()) {
			throw new IMTPException("No ICP active");
		} else {
			localAddr = URLs.get(0);
			...
			try {
				...
				if ((t != null) && (t instanceof UnreachableException)) {
					...
					logger.log(Logger.INFO,
							"No master Main Container found at address " + masterPMAddr + ". Take the leadership");
					masterPMAddr = null;
	...
	/**
	 * In the Profile ICPS can be specified as follows. 1) If there is only one ICP
	 * --> icps = <icp-class-name> <param-key> = <param-value> ....... 2) In general
	 * (typically used when there are 2 or more ICPs) icps =
	 * <icp1-class-name>(<icp1-id>);<icp2-class-name>(<icp2-id>)...
	 * <icp1-id>-<param-key> = <param-value> ....... <icp2-id>-<param-key> =
	 * <param-value> .......
	 * 
	 * If there is no ICP indication set it as follows. a) Peripheral container /
	 * J2SE or PJAVA - There is at least 1 ICP already active --> Nothing - There
	 * are no ICPs already active --> JICPPeer b) Peripheral container / MIDP -->
	 * Throws a ProfileException c) Main container / J2SE or PJAVA --> JICPPeer
	 */
	private void setDefaults() throws ProfileException {
		// - MAIN URL (Useful for peripheral containers and backup main. Ignored
		// otherwise
		String mainURL = theProfile.getParameter(MAIN_URL, null);
		if (mainURL == null) {
