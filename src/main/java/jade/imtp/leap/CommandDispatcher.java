...
import jade.imtp.leap.exception.DispatcherException;
import jade.imtp.leap.exception.ICPException;
import jade.imtp.leap.exception.LEAPSerializationException;
import jade.mtp.TransportAddress;
import jade.util.Logger;

/**
 * This class provides the implementation of a command dispatcher. The command
 * dispatcher misses support for multiple remote objects, multiple ICPs and
 * command routing.
 * 
 * <p>
 * The command dispatcher is based on an implementation written by Michael
 * Watzke and Giovanni Caire (TILAB), 09/11/2000.
 * </p>
 * 
...
class CommandDispatcher implements StubHelper, ICP.Listener {
	...
	/**
	 * The default name for new instances of the class <tt>CommandDispatcher</tt>
	 * that have not get an unique name by their container yet.
	 */
	...
	/**
	 * The singleton map of command dispatchers in case more than one must be
	 * present in the same JVM.
	 */
	...
	/**
	 * The unique name of this command dispatcher used to avoid loops in the
	 ...
	 * dispatcher to the skeletons for these objects. It is used when a command is
	 * received from a remote JVM.
	 */
	protected Map skeletons = new HashMap<>();

	/**
	 * This hashtable maps the objects remotized by this command dispatcher to their
	 * IDs. It is used when a stub of a remotized object must be built to be sent to
	 * a remote JVM.
	 */
	protected Map ids = new HashMap<>();

	/**
	 * A counter that is used for determining IDs for remotized objects. Everytime a
	 * new object is registered by the command dispatcher it gets the value of this
	 * field as ID and the field is increased.
	 */
	protected int nextID;

	/**
	 * The pool of ICP objects used by this command dispatcher to actually
	 * send/receive data over the network. It is a table that associates a
	 * <tt>String</tt> representing a protocol (e.g. "http") to a list of ICPs
	 * supporting that protocol.
	 */
	protected Map icps = new HashMap<>();

	/**
	 * The transport addresses the ICPs managed by this command dispatcher are
	...
	/**
	 * Returns a reference to the singleton instance of the CommandDispatcher for
	 * the indicated platform. Such instance is created if necessary. If no platform
	 * name is specified or the enableMultiplePlatforms flag is not set, then use
	...
	/**
	 * A sole constructor. To get a command dispatcher the constructor should not be
	 * called directly but the static <tt>create</tt> and <tt>getDispatcher</tt>
	 * methods should be used. Thereby the existence of a singleton instance of the
	 * command dispatcher will be guaranteed.
	 */
	...
	// This is only called when reconnecting to a new Master Main Container --> No
	// need to check again the platformName
	...
	/**
	 * Sets the transport address of the default router used for the forwarding
	 * mechanism.
	 * 
	...
	/**
	 * This method dispatches the specified command to the first address (among
	 * those specified) to which dispatching succeeds.
	 * 
	 * @param destTAs a list of transport addresses where the command dispatcher
	 *                should try to dispatch the command.
	 ...
	 * @throws UnreachableException if none of the destination addresses is
	 *                              reachable.
	 */
	public Command dispatchCommand(List<TransportAddress> destTAs, Command command) throws DispatcherException, UnreachableException {
		...
		// System.out.println("Dispatching command of type " + command.getCode() + " to
		// "+ta.getHost()+":"+ta.getPort());
		...
		if (response == null) {
			try {
				response = dispatchSerializedCommand(destTAs, serializeCommand(command),
						command.getRequireFreshConnection(), name);
	 ...
	 * addresses (the first where dispatching succeeds) directly or through the
	 * router.
	 * 
	 * @param destTAs        a list of transport addresses where the command
	 *                       dispatcher should try to dispatch the command.
	 * @param commandPayload the serialized command that is to be dispatched.
	 * @param origin         a <tt>String</tt> object describing the origin of the
	 ...
	 *                              reachable.
	 */
	private Command dispatchSerializedCommand(List<TransportAddress> destTAs, byte[] commandPayload, boolean requireFreshConnection,
			String origin) throws DispatcherException, UnreachableException {
		// Be sure that the destination addresses are correctly specified
		if (destTAs == null || destTAs.isEmpty()) {
	...
	/**
	 * Dispatches the specified serialized command to one of the specified transport
	 * addresses (the first where dispatching succeeds) directly.
	 * 
	 * @param destTAs        a list of transport addresses where the command
	 *                       dispatcher should try to dispatch the command.
	 * @param commandPayload the serialized command that is to be dispatched.
	 * @return a serialized response command from the receiving container.
	 * @throws UnreachableException if none of the destination addresses is
	 *                              reachable.
	 */
	private byte[] dispatchDirectly(List<TransportAddress> destTAs, byte[] commandPayload, boolean requireFreshConnection)
			throws UnreachableException {

		// Loop on destination addresses (No need to check again
		// that the list of addresses is not-null and not-empty)
		UnreachableException lastException = null;
		for (int i = 0; i < destTAs.size(); i++) {
			try {
				return send(destTAs.get(i), commandPayload, requireFreshConnection);
			} catch (UnreachableException ue) {
				lastException = ue;
				// Can't send command to this address --> try the next one
				// DEBUG
				// TransportAddress ta = (TransportAddress)destTAs.get(i);
				// System.out.println("Sending command to " + ta.getProto() + "://" +
				// ta.getHost() + ":" + ta.getPort() + " failed [" + ue.getMessage() + "]");
				// if (i < destTAs.size() - 1)
	...
	/**
	 * Dispatches the specified serialized command to one of the specified transport
	 * addresses (the first where dispatching succeeds) through the router.
	 * 
	 * @param destTAs        a list of transport addresses where the command
	 *                       dispatcher should try to dispatch the command.
	 * @param commandPayload the serialized command that is to be dispatched.
	 ...
	 *                              reachable.
	 */
	private byte[] dispatchThroughRouter(List destTAs, byte[] commandPayload, boolean requireFreshConnection,
			String origin) throws DispatcherException, UnreachableException {
	...
	/**
	 * Checks whether some exceptions to be handled by the command dispatcher
	 * occurred on the remote site. If this is the case the command dispatcher
	 * throws the corresponding exception locally.
	 * 
	 * @param response the resonse comman from the receiving container.
	 * @throws DispatcherException  if an error occurs on the remote site during
	 *                              dispatching.
	 * @throws UnreachableException if the destination address is not reachable.
	 */
	 ...
	 * @throws LEAPSerializationException if the command cannot be serialized.
	 */
	 ...
	 * @throws LEAPSerializationException if the command cannot be deserialized.
	 */
	 ...
	 * @param args the arguments required by the ICP for the activation. These
	 *             arguments are ICP specific.
	 */
		...
		try {
			...
			if (myLogger.isLoggable(Logger.FINE)) {
				myLogger.log(Logger.FINE,
						"ICP " + peerID + " of class " + peer.getClass().getName() + " activated. Address is: " + url);
			}
			addresses.add(ta);
			urls.add(url);
	 ...
	 * @throws RuntimeException if the specified object is not remotized by this
	 *                          command dispatcher.
	 */
	...
	/**
	 * Converts an URL into a transport address using the transport protocol
	 * supported by the ICPs currently installed in the command dispatcher. If there
	 * is no ICP installed to the command dispatcher or their transport protocols
	 * are not able to convert the specified URL a <tt>DispatcherException</tt> is
	 * thrown.
	 * 
	 * @param url a <tt>String</tt> object specifying the URL to convert.
	 * @return the converted URL.
	 * @throws DispatcherException if there is no ICP installed to the command
	 *                             dispatcher or the transport protocols of the ICPs
	 ...
	 * 
	 * @param skeleton     a skeleton to be managed by the command dispatcher.
	 * @param remoteObject the remote object related to the specified skeleton.
	 */
	public synchronized void registerSkeleton(Skeleton skeleton, Object remotizedObject) throws IMTPException {
		...
		if (myLogger.isLoggable(Logger.FINE)) {
			myLogger.log(Logger.FINE,
					"Registering skeleton " + skeleton + " for remotized object " + remotizedObject + ". ID is " + id);
		}
	...
	public synchronized void deregisterSkeleton(final Object remoteObject) {
		...
		if (skeletons.size() == 1) {
			// This is the only skeleton --> The problem described below (in the "else"
			// clause)
			// can't happen. Moreover the JVM is likely going to exit --> We can't delay the
			// skeleton deregistration
	...
	private synchronized void removeRemoteObject(Object remoteObject) {
		...
		if (id != null) {
			if (myLogger.isLoggable(Logger.FINE)) {
				myLogger.log(Logger.FINE, "Asynchronous deregisteration of skeleton for remotized object "
						+ remoteObject + ". ID is " + id);
			}
			skeletons.remove(id);
		}

		// When there are no more skeletons, shutdown the CommandDispatcher (this closes
		// all ICPs)
		if (ids.isEmpty()) {
			if (myLogger.isLoggable(Logger.FINE)) {
	...
	public Stub buildLocalStub(Object remotizedObject) throws IMTPException {
		...
		Iterator<TransportAddress> it = addresses.iterator();
		while (it.hasNext()) {
			stub.addTA(it.next());
		}
	...
	/**
	 * Selects a suitable peer and sends the specified serialized command to the
	 * specified transport address.
	 * 
	 * @param ta             the transport addresses where the command should be
	 *                       sent.
	 * @param commandPayload the serialized command that is to be sent.
	 * @return a serialized response command from the receiving container.
	 * @throws UnreachableException if the destination address is not reachable.
	 */
	private byte[] send(TransportAddress ta, byte[] commandPayload, boolean requireFreshConnection)
			throws UnreachableException {
		...
		for (int i = 0; i < list.size(); i++) {
			try {
				...
				// System.out.println("Warning: can't deliver command to "+ta+".
				// "+icpe.getMessage());
			}
		}
	...
	/**
	 * Handles a received (still serialized) command object, i.e. deserialize it and
	 * launch processing of the command.
	 * 
	 * @param commandPayload the command to be deserialized and processed.
	 * @return a <tt>byte</tt> array containing the serialized response command.
	 * @throws LEAPSerializationException if the command cannot be (de-)serialized.
	 */
			...
			if (command.getCode() == Command.FORWARD) {
				...
				if (origin.equals(name)) {
					...
					response = buildExceptionResponse(
							new UnreachableException("destination unreachable (and forward loop)."));
				} else {
					try {
