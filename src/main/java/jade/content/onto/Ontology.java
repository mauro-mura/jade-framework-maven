...
import jade.content.onto.exception.UnknownSlotException;
...
/**
 * An application-specific ontology describes the elements that agents can use
 * within content of messages. It defines a vocabulary and relationships between
 * the elements in such a vocabulary. The relationships can be:
 * <ul>
 ...
 * <code>Person</code>.
 * </ul>
 * Application-specific ontologies are implemented through objects of class
 * <code>Ontology</code>.<br>
 ...
 * and predicates that are allowed in messages. For example, <code>People</code>
 * ontology contains an element schema called <code>Person</code>. This schema
 * states that a <code>Person</code> is characterized by a <code>name</code> and
 * by an <code>address</code>: <code>
 ...
 * </code> where <code>PERSON<code>, <code>NAME</code> and <code>ADDRESS</code>
 * are string constants. When you register your schema with the ontology, such
 ...
 * </code> Each element schema can be associated with a Java class to map
 * elements of the ontology that comply with a schema with Java objects of that
 * class. The following is a class that might be associated with the
 * <code>Person</code> schema: <code>
 ...
 *       }
 * }
 * </code> When sending/receiving messages you can represent your content in
 * terms of objects belonging to classes that the ontology associates with
 * schemas.<br>
 ...
 * you cannot 'assign' a variable to an attribute of an object, i.e. you cannot
 * write something like: <code>person.setName(new Variable("X"))</code>.<br>
 * In order to solve this problem, you can describe your content in terms of
 * <i>abstract descriptors</i>. An abstract descriptor is an object that reifies
 * an element of the ontology. The following is the creation of an abstract
 * descriptor for a concept of type <code>Man</code>: <code>
 * AbsConcept absMan = new AbsConcept(MAN);
 * absMan.setSlot(NAME,    "John");
 * absMan.setSlot(ADDRESS, absAddress);
 * </code> where <code>absAddress</code> is the abstract descriptor for John's
 * address: <code>
 * AbsConcept absAddress = new AbsConcept(ADDRESS);
 * absAddress.setSlot(CITY, "London");
 * </code> Objects of class <code>Ontology</code> allows you to:
 * <ul>
 * <li>register schemas with associated (i) a mandatory term of the vocabulary
 * e.g. <code>NAME</code> and (ii) an optional Java class, e.g.
 * <code>Person</code>;
 * <li>retrieve the registered information through various keys.
 * </ul>
 * The framework already provides the <code>BasicOntology</code> ontology that
 * provides all basic elements, i.e. primitive data types, aggregate types, etc.
...
public class Ontology implements Serializable {
	...
	// Maps type-names to java classes
	private Map<String, Object> classes = new HashMap<>();
	// Maps java classes to schemas
	private Map<Object, ObjectSchema> schemas = new HashMap<>(); 

	private Map<String, String> conceptSlots;
	...
	/**
	 * Construct an Ontology object with a given <code>name</code> that extends a
	 * given ontology. The <code>ReflectiveIntrospector</code> is used by default to
	...
	/**
	 * Construct an Ontology object with a given <code>name</code> that uses a given
	...
	/**
	 * Construct an Ontology object with a given <code>name</code> that extends a
	 * given ontology and that uses a given Introspector to convert between Java
	 * objects and abstract descriptors.
	 * 
	 * @param name         The identifier of the ontology.
	 * @param base         The base ontology.
	 * @param introspector The introspector.
	...
	/**
	 * Construct an Ontology object with a given <code>name</code> that extends a
	 * given set of ontologies and that uses a given Introspector to convert between
	 * Java objects and abstract descriptors.
	 * 
	 * @param name         The identifier of the ontology.
	 * @param base         The base ontology.
	 * @param introspector The introspector.
			...
			if (!absObjectClass.isAssignableFrom(javaClass)) {
				...
				if (!javaClass.isInstance(schema.newInstance())) {
					throw new OntologyException(
							"Java class " + javaClass.getName() + " can't represent instances of schema " + schema);
				}
			}
		}
	}
	...
	/**
	 * Retrieves the schema of element <code>name</code> in this ontology. The
	 * search is extended to the base ontologies if the schema is not found.
	 * 
	...
	/**
	 * Retrieves the schema associated to a given class in this ontology. The search
	 * is extended to the base ontologies if the schema is not found.
	 * 
	 * @param clazz the class whose associated schema must be retrieved.
	 * @return the schema associated to the given class or <code>null</code> if the
	 *         schema is not found.
	 * @throws OntologyException
	 */
	 ...
	 * @return the object
	 * @throws UngroundedException if the abstract descriptor contains a variable
	...
	public Object toObject(AbsObject abs) throws OntologyException {
		...
		try {
			...
			// This exception may have been thrown due to the fact that the Abs descriptor
			// is
	...
	/**
	 * Retrieves the concrete class associated with element <code>name</code> in
	 * this ontology. The search is extended to the base ontologies
	 * 
	 * @param name the name of the schema.
	 * @return the Java class or null if no schema called <code>name</code> is found
	 *         or if no class is associated to that schema.
	 ...
	 * 
	 * @param lcName The lower-case version of the name of the schema whose defining
	 *               ontology must be retrieved
	 * @return The ontology actually containing the definition of schema
	 *         <code>lcName</code> or null if such schema is not defined neither in
	 *         this ontology nor in one of its super-ontologies
	 */
	 ...
	 *                   aconverted into lower case. This is passed as parameters to
	 *                   avoid making the conversion to lower case for each base
	 *                   ontology.
	 * @param globalOnto The ontology this ontology is part of (i.e. the ontology
	 *                   that extends this ontology).
	 * @return the object
	 * @throws UnknownSchemaException If no schema for the abs descriptor to be
	 *                                translated is defined in this ontology.
	 * @throws UngroundedException    if the abstract descriptor contains a variable
	 * @throws OntologyException      if some mismatch with the schema is found *
	 *                                ontology. In this case UnknownSchema
	 */
	...
	private Object internalizeSpecialType(AbsObject abs, ObjectSchema schema, Class<?> javaClass, Ontology globalOnto)
			throws OntologyException {
		if (introspector == null) {
	...
	/**
	 * Internalize (abs --> obj) the slots defined in <code>schema</code> and its
	 * super-schemas
	 */
	protected void internalize(AbsObject abs, Object obj, ObjectSchema schema, Ontology globalOnto)
			throws OntologyException {
			...
			for (int i = 0; i < names.length; ++i) {
				...
				if (absSlotValue != null) {
					...
					if (absSlotValue.getAbsType() == AbsObject.ABS_AGGREGATE) {
						// Manage as aggregate
						slotValue = introspector.internalizeAggregate(slotName, (AbsAggregate) absSlotValue, schema,
								globalOnto);
					} else {
	 ...
	 *                   that extends this ontology).
	 * @return the abstract descriptor.
	 * @throws UnknownSchemaException If no schema for the object to be translated
	 *                                is defined in this ontology.
	 * @throws OntologyException      if some mismatch with the schema is found
	 */
	...
	private AbsObject externalizeSpecialType(Object obj, ObjectSchema schema, Class<?> javaClass, Ontology globalOnto)
			throws OntologyException {
		if (introspector == null) {
			throw new NotASpecialType();
		}
	...
	/**
	 * Externalize (obj --> abs) the slots defined in <code>schema</code> and its
	 * super-schemas
	 */
	protected void externalize(Object obj, AbsObject abs, ObjectSchema schema, Ontology globalOnto)
			throws OntologyException {
		// Let the proper ontology manage slots defined in super schemas if any
		ObjectSchema[] superSchemas = schema.getSuperSchemas();
		for (int i = 0; i < superSchemas.length; ++i) {
	...
	/**
	 * Set the value of slot <code>slotName</code> as <code>slotValue</code> to
	...
	private void setSlotValue(String slotName, Object slotValue, Object obj, ObjectSchema schema)
			throws OntologyException {
		if (schema.isOwnSlot(slotName)) {
			// The slot is defined in "schema" --> use the Introspector of the ontology
			// actually defining "schema"
	...
	/**
	 * Check whether a given object is a valid term. If it is an Aggregate (i.e. a
	 * <code>List</code>) it also check the elements.
	 * 
	...
	public static void checkIsTerm(Object obj) throws OntologyException {
		// FIXME: This method is likely to be removed as it does not add any value and
		// creates problems
		 ...
		 * 
		 * obj instanceof Date || obj instanceof Term) { return; } if (obj instanceof
		 * List) { Iterator it = ((List) obj).iterator(); while (it.hasNext()) {
		 * checkIsTerm(it.next()); } return; }
		 * 
		 * // If we reach this point the object is not a term throw new
		 * OntologyException("Object "+obj+" of class "+obj.getClass().getName()
		 * +" is not a term");
		 */
	}
	...
	/**
	 * Retrieve the names of the concepts defined in this ontology only (excluding
	 * extended ontologies). It should be noticed that an agent-action is itself a
	 * concept and therefore the returned list also includes names of agent-actions
	 * defined in this ontology. <br>
	 * <b>NOT available in J2ME</b> <br>
	 * 
	 * @return the names of the concepts defined in this ontology only (excluding
	 *         extended ontologies)
	 */
	...
	/**
	 * Retrieve the names of all concepts defined in this ontology (including
	 * extended ontologies). It should be noticed that an agent-action is itself a
	 * concept and therefore the returned list also includes names of agent-actions
	 * defined in this ontology. <br>
	 * <b>NOT available in J2ME</b> <br>
	 * 
	 * @return the names of all concepts defined in this ontology (including
	 *         extended ontologies)
	 */
	...
	/**
	 * Retrieve the names of the agent actions defined in this ontology only
	 * (excluding extended ontologies). <br>
	 * <b>NOT available in J2ME</b> <br>
	 * 
	 * @return the names of the agent actions defined in this ontology only
	 *         (excluding extended ontologies)
	 */
	...
	/**
	 * Retrieve the names of all agent actions defined in this ontology (including
	 * extended ontologies). <br>
	 * <b>NOT available in J2ME</b> <br>
	 * 
	 * @return the names of all agent actions defined in this ontology (including
	 *         extended ontologies)
	 */
	...
	/**
	 * Retrieve the names of the predicates defined in this ontology only (excluding
	 * extended ontologies). <br>
	 * <b>NOT available in J2ME</b> <br>
	 * 
	 * @return the names of the predicates defined in this ontology only (excluding
	 *         extended ontologies)
	 */
	...
	/**
	 * Retrieve the names of all predicatess defined in this ontology (including
	 * extended ontologies). <br>
	 * <b>NOT available in J2ME</b> <br>
	 * 
	 * @return the names of all predicatess defined in this ontology (including
	 *         extended ontologies)
	 */
	...
	private Set<String> getElementNames(Class<?> c) {
		...
		for (int i = 0; i < base.length; ++i) {
			Ontology o = base[i];
			// Do not consider elements defined in the BasicOntology and in the
	...
	/**
	 * Create a ConceptSlotFunction for a given slot of a given Concept. The
	 * ConceptSlotFunction class allows treating the slots of an ontological concept
	 * as functions. For instance, if an ontology defines a concept
	 * <code>Person</code> with a slot <code>name</code> and a slot
	 * <code>age</code>, it is possible to create expression such as<br>
	 * (= (age (Person :name John)) 41) <br>
	 * (> (age (Person :name John)) (age (Person :name Bill)))<br>
	 * (iota ?x (= (age (Person :name John)) ?x)) <br>
	 * <b>NOT available in MIDP</b> <br>
	 * 
	 * @param slotName The name of the slot
	 * @param c        The concept a ConceptSlotFunction must be created for. This
	 *                 concept must have a slot called <code>slotName</code>
		...
		if (schema != null) {
			if (conceptSlots != null) {
				if (schema.containsSlot(slotName)) {
					...
					throw new OntologyException("Schema " + schema.getTypeName() + " for class " + c.getClass()
							+ " does not contain a slot called " + slotName);
				}
			} else {
				throw new OntologyException(
						"Ontology " + name + " does not support usage of concept slots as functions");
			}
		} else {
			for (int i = 0; i < base.length; ++i) {
				try {
	...
	/**
	 * Instruct this ontology to support usage of concept slots as functions. This
	 * method must be invoked after all schemas have been completely defined and
	...
	protected void useConceptSlotsAsFunctions() {
		...
		Iterator<ObjectSchema> it = schemas.values().iterator();
		while (it.hasNext()) {
			ObjectSchema schema = it.next();
	...
	public static AbsObject externalizeSlotValue(Object obj, Introspector introspector, Ontology referenceOnto)
			throws OntologyException {
		try {
	...
	public static Object internalizeSlotValue(AbsObject abs, Introspector introspector, Ontology referenceOnto)
			throws OntologyException {
