...
/**
 * This ontology allows dealing with Java Serializable objects as if they were
 * instances of ontological elements.
 * 
...
public class SerializableOntology extends Ontology {
	...
	private SerializableOntology() {
		...
		try {
			...
			PrimitiveSchema stringSchema = (PrimitiveSchema) BasicOntology.getInstance()
	...
	protected Object toObject(AbsObject abs, String lcType, Ontology globalOnto)
		...
		if (SERIALIZABLE.equals(abs.getTypeName())) {
			try {
				...
				byte[] value = Base64.getDecoder().decode(stringValue.getBytes("US-ASCII"));
				...
				/*
				 * #J2ME_INCLUDE_BEGIN ObjectInput in = new ObjectInputStream(new
				 * ByteArrayInputStream(value)); #J2ME_INCLUDE_END
				 */
	...
	protected AbsObject fromObject(Object obj, Ontology globalOnto) throws UnknownSchemaException, OntologyException {
		...
		if (obj instanceof Serializable) {
			try {
				...
				String stringValue = new String(Base64.getEncoder().encode(baos.toByteArray()), "US-ASCII");
