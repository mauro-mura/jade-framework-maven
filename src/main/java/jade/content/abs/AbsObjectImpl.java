...
/**
 * Base class for all non-primitive abstract descriptor classes. This class is
 * not intended to be used by programmers.
 * 
...
public class AbsObjectImpl implements AbsObject {
	 ...
	 * @param value The new value of the attribute. If <code>value</code> is null
	 *              the current mapping with <code>name</code> (if any) is removed.
	 */
	...
	/**
	 * Gets the value of an attribute of the object held by this abstract
	 * descriptor.
	 * 
	...
	protected AbsObject getAbsObject(int index) {
		String name = orderedKeys.get(index);
	...
	protected AbsObject[] getValues() {
		...
		for (int i = 0; i < orderedKeys.size(); ++i) {
			values[i] = getAbsObject(orderedKeys.get(i));
		}
		return values;
	}
	...
	/**
	 * Tests if the object is grounded, i.e., if no one of its attributes is
	...
	public boolean isGrounded() {
		for (Iterator<AbsObject> i = elements.values().iterator(); i.hasNext();) {
	...
	/**
	 * This method is here just for debugging. Notice that it is highly
	 * innefficient. The method StringCodec.encode() should be used instead.
	 * 
	...
	/**
	 * Returns true if the attribute is equal to this abstract descriptor, based on
	...
	/**
	 * Returns an integer hashcode calculated from the contents of this abstract
	 * descriptor
	 */
	public int hashCode() {
		// if this object is changed, then recompute its hash, otherwise use the
		// previous value
		if (changed) {
			hashCode = f(this);
			changed = false;
	...
	/**
	 * Calculates the hashcode according to a formula based on the slot names and
	 * values taken in an lexicographical order
	 */
