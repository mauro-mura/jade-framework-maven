...
public class ams extends Agent /*implements AgentManager.Listener*/ {
	...
	// CREATE AGENT
	public void createAgentAction(final CreateAgent ca, final AID requester, final JADEPrincipal requesterPrincipal, final Credentials requesterCredentials) throws FIPAException {
	...
	// CLONE AGENT
	public void cloneAgentAction(CloneAction ca, AID requester) throws FIPAException {
	...
	// MOVE AGENT
	public void moveAgentAction(MoveAction ma, AID requester) throws FIPAException {
	...
	// KILL CONTAINER
	public void killContainerAction(final KillContainer kc, final AID requester, final JADEPrincipal requesterPrincipal, final Credentials requesterCredentials) throws FIPAException {
	...
	public synchronized void shutdownPlatformAction(ShutdownPlatform sp, final AID requester, final JADEPrincipal requesterPrincipal, final Credentials requesterCredentials) throws FIPAException {
	...
	// REGISTER
	public void registerAction(Register r, AID requester) throws FIPAException {
	...
	// DEREGISTER
	public void deregisterAction(Deregister d, AID requester) throws FIPAException {
	...
	// MODIFY
	public void modifyAction(Modify m, AID requester) throws FIPAException {
	...
	public APDescription getDescriptionAction(AID requester) {
		...
		MTPDescriptor dr;
		for (Iterator<MTPDescriptor> mtps = platformMTPs().iterator(); mtps.hasNext();) {
			dr = mtps.next();
			...
			if (current != null) {
				...
				try {
					...
					for (int i = 0; i < ids.length; i++) {
						...
						Iterator<MTPDescriptor> mtps = myPlatform.containerMTPs(cid).iterator();
						while (mtps.hasNext()) {
							AddedMTP amtp = new AddedMTP();
							amtp.setAddress(mtps.next().getAddresses()[0]);
							amtp.setWhere(cid);
		...
		public EventManager() {
			handlers.put(RemovedContainer.NAME, new Handler() {
				public void handle(Event ev) {
					...
					ACLMessage notification = pendingRemovedContainers.remove(cid);
			...
			handlers.put(DeadAgent.NAME, new Handler() {
				public void handle(Event ev) {
					...
					ACLMessage notification = pendingDeadAgents.remove(agentID);
					if (notification != null) {
						send(notification);
					}
				}
			...
			handlers.put(MovedAgent.NAME, new Handler() {
				public void handle(Event ev) {
					...
					ACLMessage notification = pendingMovedAgents.remove(agentID);
					if (notification != null) {
						send(notification);
					}
				}
			...
			handlers.put(BornAgent.NAME, new Handler() {
				public void handle(Event ev) {
					...
					ACLMessage notification = pendingNewAgents.remove(agentID);
					if (notification == null) {
						notification = pendingClonedAgents.remove(agentID);
					}
					if (notification != null) {
						send(notification);
					}
	...
	public void deadAgent(PlatformEvent ev) {
		...
		if (ev.getContainerRemoved()) {
			da.setContainerRemoved(Boolean.valueOf(true));
		}
...
//	}
	 ...
	 */
	private List<MTPDescriptor> platformMTPs() {
		List<MTPDescriptor> mtps = new ArrayList<>();
		ContainerID[] cc = myPlatform.containerIDs();
		for (int i = 0; i < cc.length; ++i) {
			try {
				List<MTPDescriptor> l = myPlatform.containerMTPs(cc[i]);
				Iterator<MTPDescriptor> it = l.iterator();
				while (it.hasNext()) {
					mtps.add(it.next());
				}
			} catch (NotFoundException nfe) {
	...
	private void sendFailureNotification(final Concept action, final Object key, final FIPAException fe) {
		addBehaviour(new OneShotBehaviour(this) {
			...
			public void action() {
				...
				if (action instanceof CreateAgent) {
					notification = pendingNewAgents.remove(key);
				} else if (action instanceof KillContainer) {
					notification = pendingRemovedContainers.remove(key);
				}
				if (notification != null) {
