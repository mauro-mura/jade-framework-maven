 ...
 * <em><b>FIPA</b></em> <em>DF</em> agent. <b>JADE</b> applications typically
 * don't use this class directly, but interact with the DF agent through
 * <em>ACL</em> message passing. The <code>DFService</code> class provides a
 * number of static methods that facilitate this task. More <em>DF</em> agents
 * can be created by application programmers to divide a platform into many
 * <em><b>Agent Domains</b></em>.
 * <p>
 * A DF agent accepts a number of optional configuration parameters that can be
 * set either as command line options or within a properties file (to be passed
 * to the DF as an argument).
 * </p>
 ...
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_autocleanup</code></td>
 * <td>If set to <code>true</code>, indicates that the DF will automatically
 * clean up registrations as soon as an agent terminates. The default is
 * <code>false</code></td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_maxleasetime</code></td>
 * <td>Indicates the maximum lease time (in millisecond) that the DF will grant
 * for agent description registrations (defaults to infinite).</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_maxresult</code></td>
 * <td>Indicates the maximum number of items found in a search operation that
 * the DF will return to the requester (defaults to 100).</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_disablevalidation</code></td>
 * <td>Turns off content validation on incoming/outgoing messages (defaults to
 * false)</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_db-default</code></td>
 * <td>If set to <code>true</code>, indicates that the DF will store its
 * catalogue into an internal HSQL database, running within the same VM. (The
 * HSQL jar files have to added to the Java CLASSPATH)</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_db-url</code></td>
 * <td>Indicates the JDBC URL of the database the DF will store its catalogue
 * into. This parameter is ignored if <code>jade_domain_df_db-default</code> is
 * set. If neither this parameter nor <code>jade_domain_df_db-default</code> is
 * specified the DF will keep its catalogue in memory.</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_db-driver</code></td>
 * <td>Indicates the JDBC driver to be used to access the DF database (defaults
 * to the ODBC-JDBC bridge). This parameter is ignored if
 * <code>jade_domain_df_db-url</code> is not set or
 * <code>jade_domain_df_db-default</code> is set.</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_db-username</code>,
 * <code>jade_domain_df_db-password</code></td>
 * <td>Indicate the username and password to be used to access the DF database
 * (default to null). These parameters are ignored if
 * <code>jade_domain_df_db-url</code> is not set or
 * <code>jade_domain_df_db-default</code> is set.</td>
 * </tr>
 * <td><code>jade_domain_df_db-cleantables</code></td>
 * <td>If set to <code>true</code>, indicates that the DF will clean the content
 * of all pre-existing database tables, used by the DF. This parameter is
 * ignored if the catalogue is not stored in a database.</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_db-abortonerror</code></td>
 * <td>If set to <code>true</code>, indicates that the DF will immediately
 * terminate in case it cannot connect to the database. This parameter is
 * ignored if the catalogue is not stored in a database.</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_kb-factory</code></td>
 * <td>Indicates the name of the factory class that should be used to create the
 * knowledge base objects for the DF. The class has to be a sub class of
 * jade.domain.DFKBFactory.</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_poolsize</code></td>
 * <td>The dimension of the pool of thread dedicated to serving registration,
 * deregistration and search requests. If <code>0</code> (default) is specified
 * then registration, deregistration and search requests are served directly by
 * the df agent Thread. This parameter is ignored when using a volatile
 * (in-memory) knowledge base.</td>
 * </tr>
 * </table>
 * 
 * <p>
 * For instance the following command line will launch a JADE main container
 * with a DF that will store its catalogue into a database accessible at URL
 * jdbc:odbc:dfdb and that will keep agent registrations for 1 hour at most.
 * 
 * <code>
 java jade.Boot -gui -jade_domain_df_db-url jdbc:odbc:dfdb -jade_domain_df_maxleasetime 3600000
 </code>
 * <p>
 * Each DF has a GUI but, by default, it is not visible. The GUI of the agent
 * platform includes a menu item that allows to show the GUI of the default DF.
 * In order to show the GUI, you should simply send the following message to
 * each DF agent: <code>(request :content (action DFName (SHOWGUI))
...
public class df extends GuiAgent implements DFGUIAdapter {
	...
	// FIPA Agent Management Specification doc num: SC00023J (6.1.4 Search
	// Constraints)
	// a negative value of maxresults indicates that the sender agent is willing to
	// receive
	...
	/**
	 * Default constructor. This constructor does nothing; however, applications can
	 * create their own DF agents using regular platform management commands.
	 * Moreover, customized versions of a Directory Facilitator agent can be built
	 * subclassing this class and exploiting its protected interface.
	 */
	public df() {
	}

	/**
	 * This method starts all behaviours needed by <em>DF</em> agent to perform its
	 * role within <em><b>JADE</b></em> agent platform.
	 */
	protected void setup() {
		...
		if (args != null && args.length > 0) {
			...
			try {
				...
				logger.log(Logger.SEVERE,
						"Agent " + getLocalName() + " - Error loading configuration from file " + args[0], e);
			}
		}
		...
		logger.log(Logger.CONFIG, "Agent " + getLocalName() + " - Max lease time = "
				+ (maxLeaseTime != null ? ISO8601.toRelativeTimeString(maxLeaseTime.getTime()) : "infinite"));
		...
		try {
			...
			if (maxResultLimit < 0) {
				...
				logger.log(Logger.WARNING, "Agent " + getLocalName()
						+ " - The maxResult parameter of the DF Search Constraints can't be a negative value. It has been set to the default value: "
						+ DEFAULT_MAX_RESULTS);
			} else if (maxResultLimit > Integer.parseInt(DEFAULT_MAX_RESULTS)) {
				logger.log(Logger.WARNING, "Agent " + getLocalName()
						+ " - Setting the maxResult of the DF Search Constraint to large values can cause low performance or system crash !!");
			}
		} catch (Exception e) {
			// Keep default
		}
		...
		// ---------- Knowledge Base ----------
		StringBuilder sb = new StringBuilder("DF KB configuration:\n");
			...
			try {
				...
				if (o instanceof DFKBFactory factory) {
					...
					logger.log(Logger.SEVERE, "Agent " + getLocalName() + " - The class " + kbFactClass
							+ " is not a valid KB factory for the DF.");
				}
			} catch (Exception e) {
				logger.log(Logger.SEVERE,
						"Agent " + getLocalName() + " - Error loading class " + kbFactClass + ". " + e);
			}
		}
		...
		if (dbDefault) {
			...
			try {
				...
				logger.log(Logger.SEVERE,
						"Agent " + getLocalName() + " - Error creating persistent KB based on internal HSQLDB database",
						e);
				if (dbAbortOnError) {
					doDelete();
					return;
					...
					sb = new StringBuilder("DF KB configuration:\n");
				}
			}
		}
		...
		if (agentDescriptions == null && dbUrl != null) {
			...
			try {
				...
				logger.log(Logger.SEVERE,
						"Agent " + getLocalName() + " - Error creating persistent KB (+url = " + getValue(dbUrl)
								+ ", driver = " + getValue(dbDriver) + ", username = " + getValue(dbUsername)
		...
		if (agentDescriptions == null) {
			...
			if (sPoolsize != null) {
				logger.log(Logger.WARNING, "Agent " + getLocalName() + " - Ignoring pool-size indication (" + sPoolsize
						+ "). Parameter not supported when using volatile KB");
				sPoolsize = null;
			}
		}
		...
		/*
		 * #PJAVA_INCLUDE_BEGIN agentDescriptions = new
		 * DFMemKB(Integer.parseInt(getProperty(MAX_RESULTS, DEFAULT_MAX_RESULTS)));
		 * #PJAVA_INCLUDE_END
		 */
		...
		mt1 = MessageTemplate.and(mt1, MessageTemplate
				.not(MessageTemplate.MatchProtocol(FIPANames.InteractionProtocol.ITERATED_FIPA_REQUEST)));
		// #PJAVA_EXCLUDE_BEGIN
		...
		// #PJAVA_EXCLUDE_END
		/*
		 * #PJAVA_INCLUDE_BEGIN DFFipaAgentManagementBehaviour fipaRequestResponder =
		 * new DFFipaAgentManagementBehaviour(this, mt1);
		 * addBehaviour(fipaRequestResponder); #PJAVA_INCLUDE_END
		 */

		// Behaviour dealing with iterated searches according to the
		// iterated-fipa-request protocol
		mt1 = MessageTemplate.and(mt, MessageTemplate.MatchOntology(FIPAManagementOntology.getInstance().getName()));
		mt1 = MessageTemplate.and(mt1,
				MessageTemplate.MatchProtocol(FIPANames.InteractionProtocol.ITERATED_FIPA_REQUEST));
		DFIteratedSearchManagementBehaviour iteratedSearchResponder = new DFIteratedSearchManagementBehaviour(this,
				mt1);
		...
		mt1 = MessageTemplate.and(MessageTemplate.MatchOntology(FIPAManagementOntology.getInstance().getName()),
		...
		agentDescriptions.setLeaseManager(new LeaseManager() {
			...
			/**
			 * Grant a lease to this request according to the policy of the DF: if the
			 * requested lease is greater than the max lease policy of this DF, then the
			 * granted lease is set to the max of the DF.
			 **/
			public Object grantLeaseTime(Object item) {
				if (maxLeaseTime != null) {
					...
					if ((lease != null && lease.getTime() > (current + maxLeaseTime.getTime()))
						...
						// the first condition of this if considers the case when the agent requestes a
						// leasetime greater than the policy of this DF. The second condition, instead,
						// considers the case when the agent requests an infinite leasetime and the
						// policy of this DF does not allow infinite leases.
		...
		// In case some subscription is already present in the KB (e.g. because the DF
		// is using a DB-based KB and
		// it has just restarted) reinitialize the SubscriptionResponder protocol.
		// NOTE that this must be done after all components taking part in the
		// subscription mechanism have been properly initialized
		...
		for (int i = 0; i < ss.size(); i++) {
			try {
				...
				logger.log(Logger.WARNING,
						"Agent " + getLocalName() + " - Error deregistering subscription of dead agent " + id.getName(),
						e);
			}
		}
	}
	...
	private boolean getBooleanProperty(String sValue, String name) {
		...
		if (sValue != null) {
			try {
				...
				logger.log(Logger.WARNING, "Agent " + getLocalName() + " - \"" + sValue
						+ "\" is not a valid value for boolean parameter " + name, e);
			}
		}
		return b;
	}
	...
	private int getIntegerProperty(String sValue, String name) {
		...
		if (sValue != null) {
			try {
				...
				logger.log(Logger.WARNING, "Agent " + getLocalName() + " - \"" + sValue
						+ "\" is not a valid value for integer parameter " + name, e);
			}
		}
		return n;
	}
	...
	/**
	 * Cleanup <em>DF</em> on exit. This method performs all necessary cleanup
	 * operations during agent shutdown.
	 */
	...
	/**
	 * Add the behaviour handling a recursive search. If constraints contains a null
	 * search_id, then a new one is generated and the new search_id is stored into
	 * searchIdCache for later check (i.e. to avoid search loops).
	 */
	private void performRecursiveSearch(List localResults, DFAgentDescription dfd, SearchConstraints constraints,
			Search action) {
	...
	private final class RecursiveSearchHandler extends AchieveREInitiator {
		 ...
		 * 
		 * @param results     The search results. Initially this includes the items
		 *                    found locally.
		 ...
		 * @param action      The original Search action. This is used as a key to
		 *                    retrieve the incoming REQUEST message.
		 */
		private RecursiveSearchHandler(List results, DFAgentDescription template, SearchConstraints constraints,
				Search action) {
			super(df.this, null);
		...
		protected Vector prepareRequests(ACLMessage request) {
			...
			if (incomingRequest != null) {
				...
				while (it.hasNext()) {
					...
					ACLMessage msg = DFService.createRequestMessage(myAgent, childDF, FIPAManagementVocabulary.SEARCH,
	 ...
	 * 
	 * @return the List of descriptions matching the template specified in the
	 *         Search action. If no description is found an empty List is returned.
	 *         In case a recursive search is required it returns null to indicate
	 *         that the result is not yet available.
	 */
	...
	/**
	 * Serve a Search action of the FIPA management ontology requested using an
	 * iterated-fipa-request protocol.
	 * 
	 * @return an iterator over the DFAgentDescription matching the specified search
	 *         template.
	 */
	...
	/**
	 * Serve the ShowGui action of the JADE management ontology. Package scoped
	 * since it is called by DFJadeAgentManagementBehaviour.
	 * 
	 * @exception FailureException If the GUI is already visible or some error
	...
	/**
	 * Serve the GetParents action of the DF-Applet ontology Package scoped since it
	...
	/**
	 * Serve the GetDescription action of the DF-Applet ontology Package scoped
	...
	/**
	 * Serve the GetDescriptionUsed action of the DF-Applet ontology Package scoped
	 * since it is called by DFAppletManagementBehaviour.
	 */
	...
	/**
	 * Serve the Federate action of the DF-Applet ontology Package scoped since it
	 * is called by DFAppletManagementBehaviour.
	 */
	...
	/**
	 * Serve the RegisterWith action of the DF-Applet ontology Package scoped since
	 * it is called by DFAppletManagementBehaviour.
	 */
	...
	/**
	 * Serve the DeregisterFrom action of the DF-Applet ontology Package scoped
	 * since it is called by DFAppletManagementBehaviour.
	 */
	...
	/**
	 * Serve the ModifyOn action of the DF-Applet ontology Package scoped since it
	 * is called by DFAppletManagementBehaviour.
	 */
	...
	/**
	 * Serve the SearchOn action of the DF-Applet ontology Package scoped since it
	 * is called by DFAppletManagementBehaviour.
	 */
	...
	/**
	 * This method returns the description of this df used to federate with the
	 * given parent
	 */
	...
	/**
	 * This method creates the DFAgent descriptor for this df used to federate with
	 * other df.
	 */
	...
	/**
	 * This method set the description of the df according to the DFAgentDescription
	 * passed. The programmers can call this method to provide a different
	 * initialization of the description of the df they are implementing. The method
	 * is called inside the setup of the agent and set the df description using a
	 * default description.
	 */
	...
	/**
	 * this method can be used to remove a parent (a DF with which this DF is
	 * federated).
	 */
	...
	/**
	 * Store the request message related to an action that is being processed by a
	 * Behaviour. This information will be used by the Behaviour to send back the
	...
	/**
	 * Send the notification related to an action that has been processed by a
	 * Behaviour.
	 */
	 ...
	 *         <li>maxResultLimit if constraints.maxResults is infinite (i.e. < 0)
	 *         or greater than maxResultLimit
	 *         <li>constraints.maxResults otherwise
	 *         </ul>
	 *         This is package-scoped since it is also used by the
	...
	/**
	 * Check if this search must be served, i.e. if it has not yet been received. In
	 * particular the value of search_id must be different from any prior value that
	 * was received. If search_id is not null and it has not yet been received,
	 * search_id is added into the cache.
	 * 
