...
class BackEndWrapper implements BackEnd {
	...
	BackEndWrapper(FrontEndContainer fec, Properties pp) throws Exception {
		...
		if (properties.getProperty(MicroRuntime.PLATFORM_KEY) != null) {
			...
			// Adjust container-name property: msisdn key takes precedence over
			// container-name key
			...
			try {
				...
				// This should never happen since agent specifiers have already been
				// successfully parsed once!
				e.printStackTrace();
			}
			// Set the container name as the mediator-ID --> This will force the BE to
			// resynch as soon as we will attach to the platform
	...
	public Object serviceInvokation(String actor, String serviceName, String methodName, Object[] methodParams)
