...
/**
 * This interface provides Agent Life Cycle management services to the platform
 * AMS.
 */
public interface AgentManager {

	/**
	 * This callback interface is implemented by the AMS in order to be notified of
	 * significant platform-level events (e.g. container added or removed, agents
	 * birth or death, mtp configuration changes, etc.).
	 */
	...
	List<MTPDescriptor> containerMTPs(ContainerID cid) throws NotFoundException;

	List<AID> containerAgents(ContainerID cid) throws NotFoundException;
	...
	// JADE actions method
	/**
	 * Create an agent on the given container. If the container-name is null then
	 * the agent is created on the main-container.
	 * 
	 ...
	 **/
	void create(String agentName, String className, Object[] arguments, ContainerID cid, JADEPrincipal owner,
	...
	void kill(AID agentID, JADEPrincipal requesterPrincipal, Credentials requesterCredentials)
	...
	void copy(AID agentID, Location where, String newAgentName)
			throws NotFoundException, NameClashException, UnreachableException, JADESecurityException;

	void shutdownPlatform(JADEPrincipal requesterPrincipal, Credentials requesterCredentials)
			throws JADESecurityException;

	void killContainer(ContainerID cid, JADEPrincipal requesterPrincipal, Credentials requesterCredentials)
			throws NotFoundException, UnreachableException, JADESecurityException;

	MTPDescriptor installMTP(String address, ContainerID cid, String className)
	...
	void take(AID agentID, String username, byte[] password)
			throws NotFoundException, UnreachableException, JADESecurityException;

	void sniffOn(AID snifferName, List<AID> toBeSniffed) throws NotFoundException, UnreachableException;

	void sniffOff(AID snifferName, List<AID> toBeSniffed) throws NotFoundException, UnreachableException;

	void debugOn(AID debuggerName, List<AID> toBeDebugged) throws NotFoundException, UnreachableException;

	void debugOff(AID debuggerName, List<AID> toBeDebugged) throws NotFoundException, UnreachableException;
	...
	List<AMSAgentDescription> amsSearch(AMSAgentDescription template, long maxResults);
}
