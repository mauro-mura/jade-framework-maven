 ...
 * notifies its registered listener using blocking calls to the ping() remote
 * method of the monitored Node.
 *
...
public class BlockingNodeFailureMonitor extends NodeFailureMonitor implements Runnable {
	...
	public void run() {
			...
			try {
				...
				if (myLogger.isLoggable(Logger.INFO)) {
					myLogger.log(Logger.INFO,
							"PING from node " + target.getName() + " returned [" + (nodeExited ? "EXIT]" : "GO ON]"));
				}
			} catch (IMTPException imtpe1) { // Connection down
				if (myLogger.isLoggable(Logger.INFO)) {
					myLogger.log(Logger.INFO,
							"PING from node " + target.getName() + " exited with exception. " + imtpe1.getMessage());
				}
		...
		// If we reach this point without being explicitly stopped the node is no longer
		// active
		if (!stopped) {
			fireNodeRemoved();
		}
