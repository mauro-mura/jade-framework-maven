	...
	private static class Row {
			...
			if (value != null) {
				try {
					...
					while ((owner != null) && (owner != me)) {
						...
						// DEBUG System.out.println("Thread "+Thread.currentThread().getName()+" start
						// waiting on "+target);
						// DEBUG b = true;
						// DEBUG }
						wait();
						// DEBUG if (b) {
						// DEBUG System.out.println("Thread "+Thread.currentThread().getName()+" stop
						// waiting on "+target);
						// DEBUG }
						if (value == null) {
							return;
		...
		public synchronized void unlock() {
			...
			if (depth == 0 || value == null) {
				...
				// DEBUG try {
				// DEBUG if (bakValue.getLocalName().equals(target)) {
				// DEBUG System.out.println("Thread "+Thread.currentThread().getName()+"
				// notifying all on "+target);
				// DEBUG }
				// DEBUG }
