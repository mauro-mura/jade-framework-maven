...
class FileMessageStorage implements MessageStorage {
		...
		for(int i = 0; i < subdirs.length; i++) {
			...
			for(int j = 0; j < files.length; j++) {
				...
				try {
					...
					ByteArrayInputStream istream = new ByteArrayInputStream(Base64.getDecoder().decode(encodedMsg.getBytes("US-ASCII"))); 
	...
	private void createMessageFile(File toStore, GenericMessage msg, AID receiver) throws IOException {
		...
		try {
			...
			p.writeObject(msg);
			String strMessage = new String(Base64.getEncoder().encode(ostream.toByteArray()), "US-ASCII");
			ostream.close();
