...
/**
 * This class is a concrete implementation of the JADE main container, providing
 * runtime support to JADE agents, and the special, front end container where
 * the AMS and the Default DF can run. This class cannot be instantiated from
 * applications. Instead, the
 * <code>Runtime.createMainContainer(Profile p)</code> method must be called.
 * 
...
public class MainContainerImpl implements MainContainer, AgentManager {
	...
	/**
	 * Profile option that specifies whether or not child nodes such as BackEnd
	 * containers managed by the BEManagementService must be considered when
	 * shutting down the platform. Considering that there can be thousands of such
	 * nodes, explicitly killing them all may lead to very long shut down time.
	...
	/**
	 * Profile option that specifies the implementation class of the DF agent. Such
	 * class must extend the jade.domain.df standard class.
	 */
	...
	public MainContainerImpl(Profile p, PlatformManagerImpl pm) throws ProfileException {
		...
		defaultDF = createDefaultDf(dfClass);
	}
	
	// Create default Directory Facilitator 
	private df createDefaultDf(String dfClass) throws ProfileException {
		df df = null;
		if (dfClass == null) {
			df = new df();
		} else {
	...
	void initSystemAgents(AgentContainer localContainer, boolean restarting)
			throws IMTPException, NotFoundException, JADESecurityException {
	...
	void startSystemAgents(AgentContainer localContainer, AMSEventQueueFeeder feeder)
	...
	void restartReplicatedAgents(AgentContainer localContainer)
			throws IMTPException, NotFoundException, JADESecurityException {
		ContainerID cid = localContainer.getID();
		NodeDescriptor dsc = getDescriptor(cid.getName());
		// The owner of the replicated agents is the owner of the main container.
		JADEPrincipal cp = dsc.getOwnerPrincipal();
		Iterator<AID> it = replicatedAgents.keySet().iterator();
		while (it.hasNext()) {
			AID aid = it.next();
			try {
				...
				if (className != null) {
					...
					myLogger.log(Logger.WARNING,
							"Missing class-name for replicated agent " + aid.getName() + ". Cannot restart it");
				}
			} catch (NameClashException nce) {
				// This agent was not in the crashed master main, but in another backup main -->
				// It is still alive
				// --> Just do nothing
			} catch (Exception e) {
	...
	/**
	 * Store the AID of a newly born agent if it has started on a remote Main
	 * Container and its class is one of those to be replicated
	 */
	private void checkReplication(AID aid, ContainerID cid) {
		try {
			...
			if (replicatedAgentClasses.contains(className) && (!cid.equals(localContainerID))
					&& getContainerNode(cid).getNode().hasPlatformManager()) {
	...
	public void installAMSBehaviour(Behaviour b) {
		theAMS.addBehaviour(b);
	}

	public void uninstallAMSBehaviour(Behaviour b) {
	 ...
	 */
	public void bornAgent(AID aid, ContainerID cid, JADEPrincipal principal, String ownership, boolean forceReplacement)
		...
		if (old != null) {
			...
			if (old.isNative()) {
				// The old agent lives in the platform. Restore it and throw a
				// NameClashException unless
				...
				// The agent lives outside the platform (neither the forceReplacement flag nor
				// the LATENT state apply in this case)
				platformAgents.put(aid, old);
				throw new NameClashException("Agent " + aid + " already registered to the platform ");
			}
		}

		// Cid is null in case of foreign or virtual agents --> These are not involved
		// in replication
		if (cid != null) {
	...
	public void movedAgent(AID agentID, ContainerID srcID, ContainerID destID) throws NotFoundException {
		...
		fireMovedAgent(srcID, destID, agentID);
	...
	public void addTool(AID tool) {
		GenericCommand cmd = new GenericCommand(jade.core.management.AgentManagementSlice.ADD_TOOL,
	...
	public void removeTool(AID tool) {
		GenericCommand cmd = new GenericCommand(jade.core.management.AgentManagementSlice.REMOVE_TOOL,
				jade.core.management.AgentManagementSlice.NAME, null);
		cmd.addParam(tool);
	 ...
	 * 
	 * @see AgentManager#create(String agentName, String className, String
	...
	public void create(String name, String className, Object[] args, ContainerID cid, JADEPrincipal owner,
		...
		GenericCommand cmd = new GenericCommand(jade.core.management.AgentManagementSlice.REQUEST_CREATE,
				jade.core.management.AgentManagementSlice.NAME, null);
	 ...
	 */
	public void kill(AID agentID, JADEPrincipal requesterPrincipal, Credentials requesterCredentials)
			throws NotFoundException, UnreachableException, JADESecurityException {
		GenericCommand cmd = new GenericCommand(jade.core.management.AgentManagementSlice.REQUEST_KILL,
				jade.core.management.AgentManagementSlice.NAME, null);
	...
	public void suspend(final AID agentID) throws NotFoundException, UnreachableException, JADESecurityException {
		GenericCommand cmd = new GenericCommand(jade.core.management.AgentManagementSlice.REQUEST_STATE_CHANGE,
				jade.core.management.AgentManagementSlice.NAME, null);
		cmd.addParam(agentID);
	 ...
	 */
	public void move(AID agentID, Location where)
			throws NotFoundException, UnreachableException, JADESecurityException {
		...
		GenericCommand cmd = new GenericCommand(jade.core.mobility.AgentMobilityHelper.REQUEST_MOVE,
				jade.core.mobility.AgentMobilitySlice.NAME, null);
		cmd.addParam(agentID);
		cmd.addParam(where);
	 ...
	 */
	public void copy(AID agentID, Location where, String newName)
			throws NotFoundException, NameClashException, UnreachableException, JADESecurityException {
		ContainerID to = (ContainerID) where;

		// Just check whether the destination exists
		getDescriptor(to.getName());

		GenericCommand cmd = new GenericCommand(jade.core.mobility.AgentMobilityHelper.REQUEST_CLONE,
				jade.core.mobility.AgentMobilitySlice.NAME, null);
		cmd.addParam(agentID);
		cmd.addParam(where);
	 ...
	 */
	public void killContainer(ContainerID cid, JADEPrincipal requesterPrincipal, Credentials requesterCredentials)
			throws NotFoundException, UnreachableException, JADESecurityException {
		GenericCommand cmd = new GenericCommand(jade.core.management.AgentManagementSlice.KILL_CONTAINER,
				jade.core.management.AgentManagementSlice.NAME, null);
		cmd.addParam(cid);
		cmd.setPrincipal(requesterPrincipal);
		cmd.setCredentials(requesterCredentials);
		...
		// If the KillContainer is directed to the local (main) container --> remove it
		// explicitly.
		// This is necessary since the local container is not monitored and therefore
		// the normal
	 ...
	 **/
	public void shutdownPlatform(JADEPrincipal requesterPrincipal, Credentials requesterCredentials)
		...
		// In facts, even if the requester does not have the permission to kill the
		// whole platform
		// auxiliary nodes would be killed anyway
		GenericCommand cmd = new GenericCommand(jade.core.management.AgentManagementSlice.SHUTDOWN_PLATFORM,
				jade.core.management.AgentManagementSlice.NAME, null);
		cmd.setPrincipal(requesterPrincipal);
		cmd.setCredentials(requesterCredentials);
		Object ret = myCommandProcessor.processOutgoing(cmd);
		...
		try {
			...
			myLogger.log(Logger.WARNING,
					"Local node " + localContainerID.getName() + " unreachable [" + ue + "]. Ignoring...");
		} catch (JADESecurityException se) {
			// Let it through
			throw se;
		} catch (Throwable t) {
			myLogger.log(Logger.WARNING,
					"Error killing local node " + localContainerID.getName() + " [" + t + "]. Ignoring...");
		}
	...
	private void shutdownContainer(ContainerID targetID, String type, JADEPrincipal requesterPrincipal,
			Credentials requesterCredentials) throws JADESecurityException {
		try {
			if (verboseShutdown) {
			...
			if (removed) {
				...
				myLogger.log(Logger.WARNING,
						type + " " + targetID.getName() + " did not terminate when requested to do so. Ignoring...");
			}
		} catch (NotFoundException nfe) {
			// Ignore the exception as we are removing a non-existing container
			if (verboseShutdown) {
			...
			myLogger.log(Logger.WARNING,
					"Error shutting down " + type + " " + targetID.getName() + "[" + t + "]. Ignoring...");
		}

		// If we get here either killContainer() threw an exception or the container did
		// not terminate.
	 ...
	 */
	public MTPDescriptor installMTP(String address, ContainerID cid, String className)
			throws NotFoundException, UnreachableException, MTPException {

		GenericCommand cmd = new GenericCommand(jade.core.messaging.MessagingSlice.INSTALL_MTP,
		 ...
		 * dsc.getAddresses().length; i++) { System.out.println("[" +
		 * dsc.getAddresses()[i] + "]"); } System.out.println("Supported Protocols: ");
	 ...
	 */
	public void uninstallMTP(String address, ContainerID cid)
			throws NotFoundException, UnreachableException, MTPException {

		GenericCommand cmd = new GenericCommand(jade.core.messaging.MessagingSlice.UNINSTALL_MTP,
				jade.core.messaging.MessagingSlice.NAME, null);
		cmd.addParam(address);
		cmd.addParam(cid);
	 ...
	 */
	public void take(final AID agentID, final String username, final byte[] password)
			throws NotFoundException, UnreachableException, JADESecurityException {
	}
	 ...
	 */
	public void sniffOn(AID snifferName, List<AID> toBeSniffed) throws NotFoundException, UnreachableException {
		GenericCommand cmd = new GenericCommand(jade.core.event.NotificationSlice.SNIFF_ON,
	 ...
	 */
	public void sniffOff(AID snifferName, List<AID> notToBeSniffed) throws NotFoundException, UnreachableException {
		GenericCommand cmd = new GenericCommand(jade.core.event.NotificationSlice.SNIFF_OFF,
				jade.core.event.NotificationSlice.NAME, null);
		cmd.addParam(snifferName);
	 ...
	 */
	public void debugOn(AID debuggerName, List<AID> toBeDebugged) throws NotFoundException, UnreachableException {
		GenericCommand cmd = new GenericCommand(jade.core.event.NotificationSlice.DEBUG_ON,
				jade.core.event.NotificationSlice.NAME, null);
	 ...
	 */
	public void debugOff(AID debuggerName, List<AID> notToBeDebugged) throws NotFoundException, UnreachableException {
		GenericCommand cmd = new GenericCommand(jade.core.event.NotificationSlice.DEBUG_OFF,
				jade.core.event.NotificationSlice.NAME, null);
		cmd.addParam(debuggerName);
	...
	/**
	 * Modify the registration of an agent to the White Pages service of this
	 * platform. If the modification implies a change in the agent state (and the
	 * agent lives in the platform) --> force that change If the modification
	 * implies a change in the agent ownership (and the agent lives in the platform)
	 * --> force that change
	 */
	public void amsModify(AMSAgentDescription dsc)
			throws NotRegistered, NotFoundException, UnreachableException, JADESecurityException {
		...
		if (ad != null) {
			...
			if (oldDsc != null) {
				...
				if (ad.isNative()) {
					...
					if (AMSAgentDescription.SUSPENDED.equals(newState)
							&& !AMSAgentDescription.SUSPENDED.equals(oldDsc.getState())) {
						suspend(agentID);
					}
					if (AMSAgentDescription.ACTIVE.equals(newState)
							&& !AMSAgentDescription.ACTIVE.equals(oldDsc.getState())) {
						activate(agentID);
					}
					if (newOwnership != null && newOwnership != oldDsc.getOwnership()) {
						/*
						 * byte[] password = Agent.extractPassword(newOwnership); String username =
						 * Agent.extractUsername(newOwnership); take(agentID, username, password);
						 */
					}
				}
				return;
	 ...
	 * template.
	 */
	public List<AMSAgentDescription> amsSearch(AMSAgentDescription template, long maxResults) {
		List<AMSAgentDescription> results = new ArrayList<>();
	 ...
	 */
	public List<MTPDescriptor> containerMTPs(ContainerID cid) throws NotFoundException {
	 ...
	 */
	public List<AID> containerAgents(ContainerID cid) throws NotFoundException {
	...
	private final boolean matchAID(AID template, AID fact) {
		...
		// 6.4.2.1'
		Iterator<AID> itTemplateAID = template.getAllResolvers();
		Iterator<AID> itFactAID = fact.getAllResolvers();

		while (itTemplateAID.hasNext()) {
			AID templateRes = itTemplateAID.next();

			// Search 'templateRes' into the remaining part of the fact sequence
			boolean found = false;
			while (!found && itFactAID.hasNext()) {
	...
	private void fireBornAgent(ContainerID cid, AID agentID, String ownership) {
		...
		for (int i = 0; i < platformListeners.size(); i++) {
			AgentManager.Listener l = platformListeners.get(i);
			try {
				l.bornAgent(ev);
			} catch (RuntimeException re) {
				re.printStackTrace();
	...
	private void removeAllAgents(ContainerID cid) {
		...
		for (int i = 0; i < allIDs.length; i++) {
			...
			if (ad != null) {
				...
				// id can be null in case of foreign agents registerd to the local AMS or
				// virtual agents
				if (id != null) {
	...
	private void removeAllMTPs(ContainerID cid) {
		try {
			List<MTPDescriptor> l = containers.getMTPs(cid);
			Object[] objs = l.toArray();
			for (int i = 0; i < objs.length; i++) {
				MTPDescriptor mtp = (MTPDescriptor) objs[i];
				GenericCommand gCmd = new GenericCommand(jade.core.messaging.MessagingSlice.DEAD_MTP,
						jade.core.messaging.MessagingSlice.NAME, null);
				gCmd.addParam(mtp);
				gCmd.addParam(cid);
