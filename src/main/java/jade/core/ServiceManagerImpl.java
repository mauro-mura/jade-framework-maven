...
/**
 * 
 * The <code>ServiceManagerImpl</code> class is the actual implementation of
 * JADE platform <i>Service Manager</i> and <i>Service Finder</i> components. It
 * holds a set of services and manages them.
 * 
...
public class ServiceManagerImpl implements ServiceManager, ServiceFinder {
	 ...
	 * profile. This constructor is package-scoped, so that only the JADE kernel is
	 * allowed to create a new Service Manager implementation.
	 * 
	 * @param p The platform profile describing how the JADE platform is to be
	 *          configured.
	 */
...
//	}
	...
	public void addNode(NodeDescriptor desc, List<ServiceDescriptor> services)
			throws IMTPException, ServiceException, JADESecurityException {
		localNodeDescriptor = desc;
		localNode = desc.getNode();
		try {
			// Install all services locally and prepare the list of services to notify to
			// the PlatformManager
			List<ServiceDescriptor> ss = new ArrayList<>(services != null ? services.size() : 0);
			if (services != null) {
				for (int i = 0; i < services.size(); ++i) {
					ServiceDescriptor sd = services.get(i);
					try {
	...
	public void removeNode(NodeDescriptor desc) throws IMTPException, ServiceException {
		// Do not notify the platform manager. The node termination will cause the
		// deregistration...
	...
	public List<Slice> findAllSlices(String serviceKey) throws IMTPException, ServiceException {
		List<Slice> v = null;
		try {
		...
		if (v == null) {
			...
			List<Slice> ss = new ArrayList<>(v.size());
			for (int i = 0; i < ss.size(); ++i) {
				ss.set(i, bindToLocalNode(v.get(i)));
			}
			return ss;
		}
	}
	...
	private void installServiceLocally(ServiceDescriptor svcDsc) throws ServiceException {
		...
		if (fIn != null) {
			if (fIn == fOut) {
				...
				throw new ServiceException(
						"The same filter object cannot be used as both incoming and outgoing filter.");
			}
	...
	/**
	 * This method implements the platform reattachement procedure that is activated
	 * after a fault and a successive recover of the Main Container.
	 * 
		...
		if (invalidPlatformManager || compareTransportAddresses(pmAddr, myPlatformManager.getLocalAddress())) {
			...
			try {
				...
				myLogger.log(Logger.INFO,
						"Re-attaching to PlatformManager at address " + myPlatformManager.getLocalAddress());
				...
				if (!name.equals(localNodeDescriptor.getName())) {
					myLogger.log(Logger.WARNING,
							"Container name changed re-attaching to PlatformManager: new name = " + name);
				}
				adjustName(name);
	...
	/**
	 * This method implements the main reconnection procedure that is activated when
	 * the main container this container is connected to crashes and a backup main
	 * container becomes the leader.
	 * 
	...
	private synchronized boolean reconnect() {
		if (status == ACTIVE_STATUS) {
			...
			try {
				...
				Iterator<String> it = backupManagers.keySet().iterator();
				while (it.hasNext()) {
					String addr = it.next();
					try {
						myPlatformManager = (PlatformManager) backupManagers.get(addr);
						myLogger.log(Logger.INFO,
								"Reconnecting to PlatformManager at address " + myPlatformManager.getLocalAddress());
	...
	public List<ServiceDescriptor> getLocalServices() {
		Collection<ServiceDescriptor> services = localServices.values();
		List<ServiceDescriptor> ss = new ArrayList<>(services.size());
		ss.addAll(services);
		return ss;
	}
