...
class MulticastMainDetectionListener implements Runnable {
	...
	private String serveGetMain(String request) throws IMTPException {
		...
		if (i > 0) {
			...
			logger.log(Logger.FINER,
					"MulticastMainDetectionListener::serveGetMain(): desired proto is \"" + proto + "\"");
		}
		...
		if (i > 0) {
			...
			request = request.substring(0, i);
			logger.log(Logger.FINER,
					"MulticastMainDetectionListener::serveGetMain(): request is for platform \"" + platformName + "\"");
		}
		...
		if (platformName != null && !platformName.equals(myPlatform)) {
			...
			logger.log(Logger.FINER, "MulticastMainDetectionListener::serveGetMain(): my platform is \"" + myPlatform
					+ "\" while request is for platform \"" + platformName + "\" --> Do not reply");
			return null;
		}

		List<TransportAddress> addresses = manager.getLocalAddresses();
		List<TransportAddress> responseAddresses = new ArrayList<>(addresses.size());

		Iterator<TransportAddress> iter = addresses.iterator();
		...
		if (responseAddresses.size() < 1) {
			...
			while (iter.hasNext()) {
				...
				response += addr.getProto() + MainDetectionManager.PROTO_ADDR_SEPARATOR + addr.getHost()
						+ MainDetectionManager.PROTO_ADDR_SEPARATOR + addr.getPort();
				if (iter.hasNext()) {
	...
	private String executeRequest(String request) throws Exception {
		...
		if (request.indexOf(MainDetectionManager.PROTO_CMD_GETMAIN) == 0) {
			...
			if (profile.isMasterMain()) {
				...
				logger.log(Logger.FINER,
						"MulticastMainDetectionListener::executeRequest(): I'm not master --> Do not reply");
			}
	...
	private String manageRequest(byte[] requestBuffer) {
		...
		try {
			logger.log(Logger.FINER,
					"MulticastMainDetectionListener::manageRequest(): request.length()=" + request.length());
			...
			while (true) {
				try {
					...
					try {
						...
						/*
						 * when stop() is called, we get here with a SocketException: Socket closed and
						 * active == false
						 */
						if (active) {
							/*
					...
					logger.log(Logger.FINER,
							"MulticastMainDetectionListener::run(): " + packet.getLength() + " bytes received");
					try {
					...
					if (response != null) {
						...
						logger.log(Logger.FINER, "MulticastMainDetectionListener::run(): sending response \"" + response
								+ "\" to " + clientAddr + ":" + responsePort);
						socket.send(packet);
					} else {
						logger.log(Logger.FINE,
								"MulticastMainDetectionListener::run(): discarded request, sending no response");
					}
				} catch (IOException ioe) {
