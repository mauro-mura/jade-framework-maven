...
/**
 * The remote proxy for the JADE kernel-level service managing the
...
public class MainReplicationProxy extends SliceProxy implements MainReplicationSlice {
	...
	@Serial
	private static final long serialVersionUID = -9156010668481886740L;
	...
	public void addReplica(String sliceName, String smAddr, int sliceIndex, NodeDescriptor dsc, List<ServiceDescriptor> services)
	...
	/*
	 * public void bornAgent(AID name, ContainerID cid, JADEPrincipal principal
	 * String ownership) throws IMTPException, NameClashException,
	 * NotFoundException, JADESecurityException { try { GenericCommand cmd = new
	 * GenericCommand(H_BORNAGENT, NAME, null); cmd.addParam(name);
	 * cmd.addParam(cid); cmd.addParam(ownership);
	 * 
	 * Node n = getNode(); Object result = n.accept(cmd); if((result != null) &&
	 * (result instanceof Throwable)) { if(result instanceof IMTPException) { throw
	 * (IMTPException)result; } else if(result instanceof NameClashException) {
	 * throw (NameClashException)result; } else if(result instanceof
	 * NotFoundException) { throw (NotFoundException)result; } else if(result
	 * instanceof JADESecurityException) { throw (JADESecurityException)result; }
	 * else { throw new IMTPException("An undeclared exception was thrown",
	 * (Throwable)result); } } } catch(ServiceException se) { throw new
	 * IMTPException("Unable to access remote node", se); } }
	 * 
	 * public void deadAgent(AID name) throws IMTPException, NotFoundException { try
	 * { GenericCommand cmd = new GenericCommand(H_DEADAGENT, NAME, null);
	 * cmd.addParam(name);
	 * 
	 * Node n = getNode(); Object result = n.accept(cmd); if((result != null) &&
	 * (result instanceof Throwable)) { if(result instanceof IMTPException) { throw
	 * (IMTPException)result; } else if(result instanceof NotFoundException) { throw
	 * (NotFoundException)result; } else { throw new
	 * IMTPException("An undeclared exception was thrown", (Throwable)result); } } }
	 * catch(ServiceException se) { throw new
	 * IMTPException("Unable to access remote node", se); } }
	 */
	 ...
	 * try { GenericCommand cmd = new GenericCommand(H_RESUMEDAGENT, NAME, null);
	 * cmd.addParam(name);
	 * 
	 * Node n = getNode(); Object result = n.accept(cmd); if((result != null) &&
	 * (result instanceof Throwable)) { if(result instanceof IMTPException) { throw
	 ...
	 * try { GenericCommand cmd = new GenericCommand(H_NEWMTP, NAME, null);
	 * cmd.addParam(mtp); cmd.addParam(cid);
	 * 
	 * Node n = getNode(); Object result = n.accept(cmd); if((result != null) &&
	 * (result instanceof Throwable)) { if(result instanceof IMTPException) { throw
	 * (IMTPException)result; } else { throw new
	 * IMTPException("An undeclared exception was thrown", (Throwable)result); } } }
	 * catch(ServiceException se) { throw new
	 * IMTPException("Unable to access remote node", se); } }
	 * 
	 * public void deadMTP(MTPDescriptor mtp, ContainerID cid) throws IMTPException
	 * { try { GenericCommand cmd = new GenericCommand(H_DEADMTP, NAME, null);
	 * cmd.addParam(mtp); cmd.addParam(cid);
	 * 
	 * Node n = getNode(); Object result = n.accept(cmd); if((result != null) &&
	 * (result instanceof Throwable)) { if(result instanceof IMTPException) { throw
	...
	/*
	 * public void deadTool(AID tool) throws IMTPException { try { GenericCommand
	 * cmd = new GenericCommand(H_DEADTOOL, NAME, null); cmd.addParam(tool);
	 * 
	 * Node n = getNode(); Object result = n.accept(cmd); if((result != null) &&
	 * (result instanceof Throwable)) { if(result instanceof IMTPException) { throw
	 * (IMTPException)result; } else { throw new
