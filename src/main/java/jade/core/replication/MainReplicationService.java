...
/**
 * A kernel-level service to manage a ring of Main Containers, keeping the
 * various replicas in sync and providing failure detection and recovery to make
 * JADE tolerate Main Container crashes.
 * 
...
public class MainReplicationService extends BaseService {
	...
	// Actions to be performed when the node monitoring system considers an
	// unreachable node as dead
	...
	public void boot(Profile p) throws ServiceException {
		try {
			...
			myLabel = slices.size() - 1;

			// Temporarily store the slices into an array...
			MainReplicationSlice[] temp = new MainReplicationSlice[slices.size()];
			// Besides notifying GADT information, the MainReplication slice that will
			// monitor this newly started
			// slice will also have to issue a NEW_NODE VCommand and a NEW_SLICE VCommands
			...
			List<ServiceDescriptor> localServices = myContainer.getServiceManager().getLocalServices();
			String localNodeName = getLocalNode().getName();
			for (int i = 0; i < slices.size(); i++) {
				try {
					MainReplicationSlice slice = (MainReplicationSlice) slices.get(i);
			...
			if (myLabel > 0) {
				myLogger.log(Logger.INFO, "Main container ring re-arranged: label = " + myLabel + " monitored label = "
						+ localSlice.monitoredLabel);
			}
		} catch (IMTPException imtpe) {
	...
	/**
	 * This method is invoked when the monitored main node remains unreachable for
	 * too long and is therefore considered dead. The default implementation simply
	 * returns REMOVE_NODE so that the node is removed. Subclasses may redefine this
	 * method to implement application-specific checks aimed at detecting whether
	 * the unreachability depends on a real death or on temporary network problems
	 * and returns different actions according to the situation.
	 * 
	 * @param unreachableNode The node that is considered dead due to long
	 *                        unreachability
	...
	/**
	 * Inner class CommandOutgoingFilter Keep tool agents information in synch among
	 * replicas
	 */
	...
	/**
	 * Inner class CommandIncomingFilter Keep agents and MTPs information in synch
	 * among replicas
	 */
	private class CommandIncomingFilter extends Filter {
		...
		private void handleInformCreated(VerticalCommand cmd)
			...
			// Avoid propagating to other slices in case the agent creation failed due to a
			// name-clash
			if (!(ret != null && ret instanceof NameClashException)) {
				...
				GenericCommand hCmd = new GenericCommand(MainReplicationSlice.H_BORNAGENT, MainReplicationSlice.NAME,
						null);
	...
	private class ServiceComponent implements Service.Slice, NodeEventListener {
		...
		private void addReplica(String sliceName, String smAddr, int sliceIndex, NodeDescriptor dsc, Vector services)
				throws IMTPException, ServiceException {
			// Get a fresh slice: in this way the address is always right (and old address
			// is overridden!!!)
			...
			if (myLabel == 0) {
				...
				// '-1' means 'all the results'
				List<AMSAgentDescription> suspendedAgents = myMain.amsSearch(amsd, -1);
				Iterator<AMSAgentDescription> it = suspendedAgents.iterator();
				while (it.hasNext()) {
					AMSAgentDescription desc = it.next();
					try {
				...
				// Finally issue a NEW_NODE VCommand and a NEW_SLICE VCommand for each service
				// to allow
				// local services to propagate service specific information to their slices in
				// the new
				// Main Container node.
				try {
			...
			myLogger.log(Logger.INFO,
					"Main container ring re-arranged: label = " + myLabel + " monitored label = " + monitoredLabel);
		}
		...
		public void nodeRemoved(Node n) {
			...
			if (monitoredNodeUnreachable) {
				// The monitored Main Container appears to be dead. Clearly we cannot
				// distinguish for sure between a
				// real death and a long network disconnection --> By redefining the
				...
				// - REMOVE_NODE: the default behaviour. Node n is actually removed and if
				// necessary the leadership is acquired
				// - WAIT: nothing is done. As soon as an ORPHAN_NODE command is intercepted for
				// node n a new monitor is activated
				// This action requires the UDPNodeMonitoringService to be active.
				// - SUICIDE: kill the local container to avoid creating problems when
			...
			case REMOVE_NODE:
				...
				break;
			case WAIT:
				myLogger.log(Logger.WARNING, "Network problems are preventing the monitoring of main node <"
						+ n.getName() + ">. Stop monitor it");
				toBeMonitored = n;
				break;
			case SUICIDE:
				suicide();
		...
		private void removeTerminatedNode(Node n) {
			...
			try {
				...
				// Possibly the AMS is dead --> Start intercepting platform and MTP events on
				// behalf of the
				// new AMS if any.
		...
		private void suicide() {
			myLogger.log(Logger.WARNING,
					"Due to network problems I'm isolated --> The rest of the platform will consider me dead. Suicide now!!!!!!!!");
			System.exit(0);
		}
		...
		private void handleOrphanNode(String nodeId) {
			if (toBeMonitored != null && toBeMonitored.getName().equals(nodeId)) {
				...
				myLogger.log(Logger.INFO, "Ping received from node " + nodeId
						+ " --> The network is working again. Re-start monitoring node");
				nodeMonitor = NodeFailureMonitor.getFailureMonitor();
