...
public class AddressNotificationService extends BaseService {
	...
	public void boot(Profile p) throws ServiceException {
		try {
			// Get the Service Manager address list, if this node isn't hosting one
			// itself...
			Node n = getLocalNode();
			if (!n.hasPlatformManager()) {
				List<Slice> slices = getAllSlices();
				for (int i = 0; i < slices.size(); i++) {
					AddressNotificationSlice slice = (AddressNotificationSlice) slices.get(i);
	...
	private class IncomingFilter extends Filter {
		...
		public void handleAddressAdded(VerticalCommand cmd) throws IMTPException, ServiceException {
			...
			GenericCommand hCmd = new GenericCommand(AddressNotificationSlice.H_ADDSERVICEMANAGERADDRESS,
					AddressNotificationSlice.NAME, null);
			hCmd.addParam(addr);
	...
	private class ServiceComponent implements Service.Slice {
		...
		private static final long serialVersionUID = 1028435064041547833L;
