...
public class ProfileImpl extends Profile {
	...
	/**
	 * This constructor creates a default Profile for launching a platform (<b>i.e.
	 * a main container!!</b>).
	 * 
	...
	/**
	 * This constructor creates a default Profile for launching a main (or non-main)
	 * container (depending on the value of <code>isMain</code>)
	 * 
	 * @param host       is the name of the host where the main-container should be
	 *                   listen to. A null value means use the default (i.e.
	 *                   localhost)
	 ...
	 *                   default. A null value means use the default (i.e.
	 *                   localhost)
	 * @param isMain     if isMain is false, then the profile is configured to
	 *                   launch a remote container, if true a main-container
	 **/
	...
	void init() {
		...
		if (localHost == null) {
			if (isMasterMain()) {
				...
				// Default for a peripheral container or a backup main container: use the local
				// host
		...
		if (isMain() && (!getBooleanProperty("nomtp", false)) && (props.getProperty(MTPS) == null)) {
			...
			List<Specifier> l = new ArrayList<>(1);
			l.add(s);
			props.put(MTPS, l);
		}
	...
	/**
	 * Retrieve the configuration properties as they were passed to this Profile
	 * object, i.e. without internal initializations automatically performed by the
	 * Profile class.
	 */
	 ...
	 */
	public List<Specifier> getSpecifiers(String key) throws ProfileException {
		...
		try {
			...
			return new ArrayList<>(Specifier.parseSpecifierList(specsLine));
