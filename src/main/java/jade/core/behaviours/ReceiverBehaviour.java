 ...
 * <code>receive()</code> as an atomic operation. This behaviour terminates when
 * an ACL message is received. The method <code>getMessage()</code> allows to
 ...
 * @author Moreno LAGO
 * @version $Date: 2003-11-25 09:24:45 +0100 (mar, 25 nov 2003) $ $Revision:
 *          4601 $
 * 
 */
public final class ReceiverBehaviour extends Behaviour {
	...
	/**
	 * Exception class for timeouts. This exception is thrown when trying to obtain
	 * an <code>ACLMessage</code> object from an <code>Handle</code>, but no message
	 * was received within a specified timeout.
	 * 
	...
	/**
	 * Exception class for timeouts. This exception is thrown when trying to obtain
	 * an <code>ACLMessage</code> from an <code>Handle</code> and no message was
	 * received so far, but the time limit is not yet reached.
	 * 
	 * @see jade.core.behaviours.ReceiverBehaviour.Handle#getMessage()
	 */
	 ...
	 * This interface is used to create a <code>ReceiverBehaviour</code> object to
	 * receive an ACL message within a user specified time limit. When the user
	 * tries to read the message represented by the handle, either gets it or gets
	 * an exception.
	 * 
	 * @see jade.core.behaviours.ReceiverBehaviour#newHandle()
	 * @see jade.core.behaviours.ReceiverBehaviour#ReceiverBehaviour(Agent a,
	 *      ReceiverBehaviour.Handle h, long millis)
	 */
	public static interface Handle {

		/**
		 * Tries to retrieve the <code>ACLMessage</code> object represented by this
		 * handle.
		 * 
		 * @return The ACL message, received by the associated
		 *         <code>ReceiverBehaviour</code>, if any.
		 * @exception TimedOut    If the associated <code>ReceiverBehaviour</code> did
		 *                        not receive a suitable ACL message within the time
		 *                        limit.
		 * @exception NotYetReady If the associated <code>ReceiverBehaviour</code> is
		 *                        still waiting for a suitable ACL message to arrive.
		 * @see jade.core.behaviours.ReceiverBehaviour#ReceiverBehaviour(Agent a,
		 *      ReceiverBehaviour.Handle h, long millis)
		 */
	 ...
	 * <code>Handle</code> object, which can be used to retrieve an ACL message out
	 * of a <code>ReceiverBehaviour</code> object.
	 * 
	...
	// A running counter for calling block(millis) until 'timeOut' milliseconds
	// pass.
	/**
	 * @serial
	 */
	...
	/**
	 * This constructor creates a <code>ReceiverBehaviour</code> object that ends as
	 * soon as an ACL message matching a given <code>MessageTemplate</code> arrives
	 * or the passed <code>millis<code> timeout expires.
	 ...
	 * @param millis The timeout expressed in milliseconds, an infinite timeout can
	 *               be expressed by a value < 0.
	...
	/**
	 * Receive any ACL message, waiting at most <code>millis</code> milliseconds
	 * (infinite time if <code>millis < 1</code>). When calling this constructor, a
	 * suitable <code>Handle</code> must be created and passed to it. When this
	 * behaviour ends, some other behaviour will try to get the ACL message out of
	 * the handle, and an exception will be thrown in case of a time out. The
	 * following example code explains this:
	 * 
	 * <code><pre>
	 ...
	 * @param millis The maximum amount of time to wait for the message, in
	 *               milliseconds.
	 * @see jade.core.behaviours.ReceiverBehaviour.Handle
	 * @see jade.core.behaviours.ReceiverBehaviour#newHandle()
	 */
	 ...
	 * <code>millis</code> milliseconds (infinite time if <code>millis < 1</code>.
	 * When calling this constructor, a suitable <code>Handle</code> must be created
	 * and passed to it.
	 * 
	 * @param a      The agent this behaviour belongs to.
	 * @param h      An <em>Handle</em> representing the message to receive.
	 ...
	 * @param mt     A Message template to match incoming messages against, null to
	 *               indicate no template and receive any message that arrives.
	 * @see jade.core.behaviours.ReceiverBehaviour#ReceiverBehaviour(Agent a, Handle
	 *      h, long millis)
	 */
	...
	/**
	 * Actual behaviour implementation. This method receives a suitable ACL message
	 * and copies it into the message provided by the behaviour creator. It blocks
	 * the current behaviour if no suitable message is available.
	 */
	public void action() {
		ACLMessage msg = null;
		if (template == null)
	 ...
	 * <code>ACLMessage</code> with the same <code>ReceiverBehaviour</code> without
	 ...
	 * @exception TimedOut    if the timeout passed in the constructor of this class
	 *                        expired before any message (that eventually matched
	 *                        the passed message template) arrived
	 * @exception NotYetReady if the message is not yet arrived and the timeout is
	 *                        not yet expired.
	 **/
	public ACLMessage getMessage() throws TimedOut, NotYetReady {
