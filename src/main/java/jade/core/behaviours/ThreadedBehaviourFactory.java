...
/**
 * This class provides support for executing JADE Behaviours in dedicated Java
 * Threads. In order to do that it is sufficient to add to an agent a normal
 * JADE Behaviour "wrapped" into a "threaded behaviour" as returned by the
 * <code>wrap()</code> method of this class (see the example below).
 * 
 * <pr>
 * <hr>
 * <blockquote>
 ...
 * <hr>
 * 
 * This class also provides methods to control the termination of the threads
 * dedicated to the execution of wrapped behaviours
 * 
 * <br>
...
public class ThreadedBehaviourFactory {
	 ...
	 * Adding the wrapper behaviour to an agent results in executing <code>b</code>
	...
	/**
	 * @return The number of active threads dedicated to the execution of wrapped
	 *         behaviours.
	 */
	public int size() {
	...
	/**
	 * Blocks until all threads dedicated to the execution of threaded behaviours
	 * complete.
	 * 
	 * @param timeout The maximum timeout to wait for threaded behaviour
	 *                termination.
	 * @return <code>true</code> if all threaded behaviour have actually completed,
	...
	/**
	 * Interrupt a threaded behaviour. This method should be used to abort a
	 * threaded behaviour instead of getThread().interrupt() because i) the latter
	 * may have no effect if called just after the threaded behaviour suspended
	 * itself and ii) the threaded behaviour may be suspended and in this case its
	 * Thread is null.
	 * 
	...
	/**
	 * Suspend a threaded behaviour. This method has only effect if called by the
	 * threaded behaviour itself and has the effect of releasing its dedicated Java
	 * Thread. This can later be restored by means of the <code>resume()</code>
	 * method.
	 */
	...
	/**
	 * Resume a threaded behaviour. Assign a new Java Thread to a threaded behaviour
	 ...
	 * 
	 * @return All the wrapper behaviours currently used by this
	...
	private ThreadedBehaviourWrapper getWrapper(Behaviour b) {
		synchronized (threadedBehaviours) {
			Enumeration<ThreadedBehaviourWrapper> e = threadedBehaviours.elements();
			while (e.hasMoreElements()) {
				ThreadedBehaviourWrapper tb = e.nextElement();
	...
	public final class ThreadedBehaviourWrapper extends Behaviour implements Runnable {
		...
		/**
		 * Propagate the parent to the wrapped behaviour. NOTE that the
		 * <code>parent</code> member variable of the wrapped behaviour must point to
		 * the DummyParentBehaviour --> From the wrapped behaviour accessing the actual
		 * parent must always be retrieved through the getParent() method.
		 */
		 ...
		 * ThreadedBehaviourWrapped is directly added to the agent Scheduler and a
		 * message is received) to the wrapped threaded behaviour.
		 */
		public void restart() {
			myBehaviour.restart();
		}
		 ...
		 * ThreadedBehaviourWrapper is added as a child of a CompositeBehaviour and the
		 * latter, or an ancestor, is blocked/restarted) to the wrapped threaded
		 * behaviour. If the event is a restart, also notify the dedicated thread.
		 */
			...
			try {
				while (true) {
					...
					synchronized (this) {
						...
						if (restarted) {
							// We can't just set the runnable state of myBehaviour to true since, if
							// myBehaviour
							// is a CompositeBehaviour, we may end up with myBehaviour runnable, but some of
							// its children not runnable.
							// However we can't call myBehaviour.restart() here because there could be a
							// deadlock between a thread
							// posting a message and the current thread (monitors are this and the agent
							// scheduler)
						...
						if (myBehaviour.done()) {
							...
							// If the Behaviour suspended itself during the action() method --> Release the
							// embedded Thread
							if (suspended) {
								threadState = SUSPENDED_STATE;
								myThread = null;
								// NOTE: We do not invoke the onSuspended() callback method here, but in the
								// finally block
								// to avoid giving users the possibility of putting application-specific code
								// inside a section
								// where we already locked the ThreadedBehaviourFactory-Wrapper. This would open
								// the door to
								// deadlock conditions.
								return;
							}
							while (!myBehaviour.isRunnable()) {
								...
								// Wake every minute: if the agent died in the meanwhile the
								// checkAgentTerminated() method
				...
				myLogger.log(Logger.WARNING,
						"Threaded behaviour " + myBehaviour.getBehaviourName() + " interrupted before termination");
				...
				threadState = INTERRUPTED_STATE;
				myLogger.log(Logger.WARNING,
						"Threaded behaviour " + myBehaviour.getBehaviourName() + " stopped before termination");
				// ThreadDeath errors should always be propagated so that the top level handler
				// can perform the necessary clean up
				throw td;
			} catch (Throwable t) {
				...
				if (threadState == SUSPENDED_STATE) {
					// If we have just suspended and the Behaviour defined a handleSuspended()
					// method
		 ...
		 * getThread().interrupt() because the latter may have no effect if called just
		 * after the threaded behaviour suspended itself
		 */
		...
		private void terminate() {
			if (Thread.currentThread() == myThread) {
				if (threadState == INTERRUPTED_STATE || threadState == ERROR_STATE) {
					// If the Behaviour defined a handleAborted() method invoke it from within the
					// terminating thread
			...
			finished = true;
			// Restart the wrapper so that it terminates too and is removed from the Agent
			// scheduler
			super.restart();
	...
	/**
	 * Inner class DummyParentBehaviour. This class has the only purpose of
	...
	private final class DummyParentBehaviour extends CompositeBehaviour {
		...
		/**
		 * Redefine the root() method so that both the DummyParentBehaviour and the
		 * ThreadedBehaviourWrapper are invisible in the behaviours hierarchy
		 */
