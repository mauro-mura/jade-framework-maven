...
/**
 * Base class for behaviours implementing a FIPA-request like pattern with a
 * single target agent.
 * 
 * @author Moreno LAGO
 */
public abstract class BaseInitiator extends AchieveREInitiator {
	...
	/**
	 * Concrete subclasses are expected to implement this method to create the
	 * initiation message.
	 * 
	...
	/**
	 * Concrete subclasses can redefine this method to provide a human readable
	 * description of the target agent of this behaviour. Such description will be
	 * used in all default error messages.
	 * 
	...
	@Override
	public Vector<ACLMessage> prepareRequests(ACLMessage msg) {
		...
		try {
			...
			if (initiation != null) {
				...
				v.add(initiation);
				Iterator<AID> it = initiation.getAllReceiver();
				if (it.hasNext()) {
					target = it.next();
	...
	public void handleRefuse(ACLMessage refuse) {
		...
		outcome.error("REFUSE response received from " + refuse.getSender().getLocalName() + ": " + refuse.getContent(),
				null);
	}

	@Override
	public void handleNotUnderstood(ACLMessage notUnderstood) {
		// Target agent replied with NOT_UNDERSTOOD (Target agent failed decoding
		// initiation message)
		outcome.error("NOT_UNDERSTOOD response received from " + notUnderstood.getSender().getLocalName() + ": "
				+ notUnderstood.getContent(), null);
	}
	...
	public void handleFailure(ACLMessage failure) {
		if (failure.getSender().equals(myAgent.getAMS()) && !target.equals(myAgent.getAMS())) {
			...
			outcome.error("FAILURE response received from " + failure.getSender().getLocalName() + ": "
					+ failure.getContent(), null);
		}
	}
	...
	public void handleAllResultNotifications(Vector notifications) {
		if (notifications.isEmpty()) {
			...
			// Other replies such as REFUSE immediately close the protocol --> We don't get
			// any notification in these cases
