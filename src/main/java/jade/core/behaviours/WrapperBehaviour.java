...
/**
 * This behaviour allows modifying on the fly the way an existing behaviour
 * object works The following piece of code provides an example where we modify
 * the <code>done()</code> method of an existing behaviour object to print on
...
public class WrapperBehaviour extends Behaviour {
	...
	public WrapperBehaviour(Behaviour wrapped) {
		...
		// Set a dummy parent to the wrapped-behaviour to allow propagation of upward
		// events and root calculations
		...
		wrappedBehaviour.setParent(new CompositeBehaviour() {
			...
			// Redefine the root() method to propagate root calculations to this
			// WrapperBehaviour
