...
/**
 * This behaviour serves behaviour-loading requests according to the
 * Behaviour-loading ontology. When an agent runs an instance of this behaviour
 * it becomes able to load and execute completely new behaviours, i.e.
 * behaviours whose code is not included in the classpath of the JVM where the
 * agent lives. Loading behaviour requests must have the
 * <code>ACLMessage.REQUEST</code> performative and must use the
 * BehaviourLoading ontology and the LEAP language.
 * 
 * <br>
...
public class LoaderBehaviour extends Behaviour {
	...
	private MessageTemplate myTemplate = MessageTemplate.and(MessageTemplate.MatchPerformative(ACLMessage.REQUEST),
	...
	/**
	 * Construct a LoaderBehaviour to be executed by a given agent and that will use
	 * a given class loader to load behaviours whose code is not embedded in the
	...
	/**
	 * Add a loaded behaviour to the agent. Subclasses may redefine this method to
	 * handle the behaviour addition operation in an application specific way.
	 * 
	...
	/**
	 * Suclasses may redefine this method to prevent the behaviour loading operation
	 * under specific conditions. This default implementation always returns
	 * <code>true</code>
	 */
	...
	private Behaviour loadFromCode(String className, final byte[] code)
	...
	private Behaviour loadFromZip(String className, byte[] zip)
			throws ClassNotFoundException, InstantiationException, IllegalAccessException {
	...
	private Behaviour load(String className, Map classes)
			throws ClassNotFoundException, InstantiationException, IllegalAccessException {
		...
		/*
		 * #J2ME_INCLUDE_BEGIN Class c = loader.loadClass(className); #J2ME_INCLUDE_END
		 */
	...
	private class HashClassLoader extends ClassLoader {
		...
		public HashClassLoader(Map ht, ClassLoader parent) {
			// #PJAVA_EXCLUDE_BEGIN
			super(parent);
			// #PJAVA_EXCLUDE_END
			/*
			 * #PJAVA_INCLUDE_BEGIN super(); #PJAVA_INCLUDE_END
			 */
			classes = ht;
		}
		...
		/*
		 * #PJAVA_INCLUDE_BEGIN In PersonalJava loadClass(String, boolean) is abstract
		 * --> we must implement it protected Class loadClass(String name, boolean
		 * resolve) throws ClassNotFoundException { // 1) Try to see if the class has
		 * already been loaded Class c = findLoadedClass(name);
		 * 
		 * // 2) Try to load the class using the system class loader if(c == null) { try
		 * { c = findSystemClass(name); } catch (ClassNotFoundException cnfe) { } }
		 * 
		 * // 3) If still not found, try to load the class from the code if(c == null) {
		 * c = findClass(name); }
		 * 
		 * if(resolve) { resolveClass(c); }
		 * 
		 * return c; } #PJAVA_INCLUDE_END
		 */
	...
	/**
	 * Inner class ResultCollector. This behaviour restores the output parameters at
	 * the end of the execution of the loaded behaviour and sends back the result
