...
/**
 * Composite behaviour with concurrent children scheduling. It is a
 * <code>CompositeBehaviour</code> that executes its children behaviours
 * concurrently, and it terminates when a particular condition on its
 * sub-behaviours is met i.e. when all children are done, <em>N</em> children
 * are done or any child is done.
 * 
...
public class ParallelBehaviour extends CompositeBehaviour {
	 ...
	 * <code>ParallelBehaviour</code> that terminates when all its children are
	 * done.
	 */
	public static final int WHEN_ALL = 0;
	/**
	 * Predefined constant to be used in the constructor to create a
	 * <code>ParallelBehaviour</code> that terminates when any of its child is done.
	 */
	public static final int WHEN_ANY = 1;

	private int whenToStop;
	private BehaviourList subBehaviours = new BehaviourList();
	private Hashtable<Behaviour, Behaviour> blockedChildren = new Hashtable<>();
	private BehaviourList terminatedChildren = new BehaviourList();

	/**
	 * Construct a <code>ParallelBehaviour</code> without setting the owner agent,
	 * and using the default termination condition (i.e. the parallel behaviour
	 * terminates as soon as all its children behaviours terminate.
	 */
	 ...
	 * Construct a <code>ParallelBehaviour</code> without setting the owner agent.
	 * 
	 * @param endCondition this value defines the termination condition for this
	 *                     <code>ParallelBehaviour</code>. Use
	 *                     <ol>
	 *                     <li><code>WHEN_ALL</code> to terminate this
	 *                     <code>ParallelBehaviour</code> when all its children are
	 *                     done.</li>
	 *                     <li><code>WHEN_ANY</code> to terminate this
	 *                     <code>ParallelBehaviour</code> when any of its child is
	 *                     done.</li>
	 *                     <li>a positive <code>int</code> value n to terminate this
	 *                     <code>ParallelBehaviour</code> when n of its children are
	 *                     done.</li>
	 *                     </ol>
	 */
	...
	/**
	 * Construct a <code>ParallelBehaviour</code> setting the owner agent.
	 * 
	 * @param a            the agent this <code>ParallelBehaviour</code> belongs to.
	 * @param endCondition this value defines the termination condition for this
	 *                     <code>ParallelBehaviour</code>. Use
	 *                     <ol>
	 *                     <li><code>WHEN_ALL</code> to terminate this
	...
	/**
	 * This method schedules children behaviours one at a time, in a round robin
	 * fashion.
	 * 
	...
	/**
	 * Return a Collection view of the children of this
	 * <code>ParallelBehaviour</code> that have already completed.
	 */
	public Collection<Behaviour> getTerminatedChildren() {
	...
	public void removeSubBehaviour(Behaviour b) {
		...
		if (!b.isRunnable()) {
			...
			if ((!subBehaviours.isEmpty()) && (blockedChildren.size() == subBehaviours.size())) {
				myEvent.init(false, NOTIFY_UP);
				super.handle(myEvent);
			}
		}
	...
	/**
	 * Resets this behaviour. This methods puts a <code>ParallelBehaviour</code>
	 * back in initial state, besides calling <code>reset()</code> on each child
	 * behaviour recursively.
	 */
	public void reset() {
	...
	/**
	 * Handle block/restart notifications. A <code>ParallelBehaviour</code> object
	 * is blocked <em>only</em> when all its children behaviours are blocked and
	 * becomes ready to run as soon as any of its children is runnable. This method
	 * takes care of the various possibilities.
	 * 
	...
	protected void handle(RunnableChangedEvent rce) {
		...
		synchronized (subBehaviours) {
			if (rce.isUpwards()) {
				...
				setRunnable(r);
				// Notify all children
				Iterator<Behaviour> it = getChildren().iterator();
				while (it.hasNext()) {
					Behaviour b = it.next();
					b.handle(rce);
				}
