...
/**
 * An abstract superclass for behaviours composed by many parts. This class
 * holds inside a number of <b><em>children behaviours</em></b>. When a
 * <code>CompositeBehaviour</code> receives it execution quantum from the agent
 * scheduler, it executes one of its children according to some policy. This
 * class must be extended to provide the actual scheduling policy to apply when
 * running children behaviours.
 * 
 ...
 * @author Moreno LAGO
 * @version $Date: 2012-06-20 11:44:30 +0200 (mer, 20 giu 2012) $ $Revision:
 *          6573 $
 * 
 */
public abstract class CompositeBehaviour extends Behaviour {
	...
	/**
	 * Executes this <code>CompositeBehaviour</code>. This method executes children
	 * according to the scheduling policy defined by concrete subclasses that
	 * implements the <code>scheduleFirst()</code> and <code>scheduleNext()</code>
	 * methods.
	 */
	 ...
	 * 
	 * @return <code>true</code> if this <code>CompositeBehaviour</code> has
	...
	/**
	 * This methods is called after the execution of each child in order to check
	 * whether the <code>CompositeBehaviour</code> should terminate.
	 * 
	 * @param currentDone   a flag indicating whether the just executed child has
	 *                      completed or not.
	 * @param currentResult the termination value (as returned by
	 ...
	 *                      case this child has completed (otherwise this parameter
	 *                      is meaningless)
	 * @return true if the <code>CompositeBehaviour</code> should terminate. false
	 *         otherwise.
	 */
	...
	/**
	 * Puts a <code>CompositeBehaviour</code> back in initial state. The internal
	 * state is cleaned up and <code>reset()</code> is recursively called for each
	 * child behaviour.
	 */
	public void reset() {
		resetChildren();
	...
	protected void resetChildren() {
		Collection<Behaviour> c = getChildren();
		if (c != null) {
			Iterator<Behaviour> it = c.iterator();
			while (it.hasNext()) {
				Behaviour b = it.next();
				b.reset();
			}
	...
	/**
	 * Associates this behaviour with the agent it belongs to. Overrides the method
	 * in the base class to propagate the setting to all children.
	 * 
	 * @param a The agent this behaviour belongs to.
