...
/**
 * Ready made behaviour that for each incoming message automatically invokes a
 * corresponding method of the form<br>
 * <code>
 * public void serveCcccPppp(Cccc c, ACLMessage msg) throws Exception
 * </code> <br>
 * where Cccc represents the key content-element referenced by the incoming
 * message msg and Pppp represents the performative of the message.<br>
 * ContentElement-s representing SL0 operators <code>action</code>,
 * <code>done</code> and <code>result</code> are automatically managed so that
 * for instance if an incoming REQUEST message is received carrying a content of
 * type<br>
...
public class OntologyServer extends CyclicBehaviour {
	...
	/**
	 * Instruct the OntologyServer to process up to maxProcessedMessagesPerRun
	 * messages each time it is scheduled for execution. This can be useful for
	 * performance reasons when the agent executing this behaviour has to deal with
	 * a heavy load of messages
	 * 
	...
	/**
	 * Set the <code>ConversationList</code> used by this OntologyServer to
	 * determine which conversations to ignore (if any). Note: calling this method
	 * after the behaviour started has no effect
	 * 
	...
	/**
	 * If an unexpected error occurs in one of the serving methods or if a suitable
	 * serving method is not found for an incoming message, the OntologyServer
	 * automatically sends back a FAILURE or REFUSE message if the incoming
	 * performative was one of REQUEST, CFP, PROPOSE, QUERY, SUBSCRIBE, PROXY and
	 * PROPAGATE. In other cases the OntologyServer simply logs a suitable error
	 * message. The above list of performatives can be customized by means of this
	 * method as well as the <code>addPerformativeRequiringReply(int)<code> one.
	 * 
	...
	public void onStart() {
		...
		// Unless a template is explicitly set, we get messages matching the ontology,
		// the served performatives.
		if (template == null) {
			if (servedPerformatives != null) {
		...
		// Whatever template is used we avoid intercepting messages belonging to
		...
		if (maxProcessedMessagesPerRun == 1) {
			...
			if (msg != null) {
				...
				if (myLogger.isLoggable(Logger.FINER)) {
					myLogger.log(Logger.FINER, "Agent " + myAgent.getName() + " - " + getBehaviourName()
							+ ": Serving incoming message " + msg);
				}
				handleMessage(msg);
			}
		} else {
	...
	protected void handleMessage(ACLMessage msg) {
		try {
			...
			if (myLogger.isLoggable(Logger.FINE)) {
				myLogger.log(Logger.FINE, "Agent " + myAgent.getName() + " - " + getBehaviourName() + ": Serving "
						+ keyCel.getClass().getName() + " " + ACLMessage.getPerformative(msg.getPerformative()));
			}
			...
			if (m != null) {
				try {
					...
					// Since we only use public methods with proper arguments this can only happen
					// if the serverDelegate class is not accessible
	...
	protected void handleUnsupported(Object keyCel, ACLMessage msg) {
		String detail = printFullUnexpectedMessages ? "Message = " + msg.toString()
				: "Sender is " + msg.getSender().getName();
		myLogger.log(Logger.WARNING, "Agent " + myAgent.getName() + " - " + getBehaviourName()
				+ ": Unsupported content-element " + keyCel.getClass().getName() + ". " + detail);
	...
	protected void handleServingFailure(Throwable t, Object cel, ACLMessage msg) {
		String detail = printFullUnexpectedMessages ? "Message = " + msg.toString()
				: "Sender is " + msg.getSender().getName();
		myLogger.log(Logger.SEVERE, "Agent " + myAgent.getName() + " - " + getBehaviourName()
				+ ": Unexpected error serving content-element " + cel.getClass().getName() + ". " + detail, t);
		if (performativesRequiringReply.contains(msg.getPerformative())) {
			ACLMessage reply = msg.createReply();
	...
	protected void handleNotUnderstood(ContentException ce, ACLMessage msg) {
		myLogger.log(Logger.WARNING,
				"Agent " + myAgent.getName() + " - " + getBehaviourName() + ": Error decoding "
						+ ACLMessage.getPerformative(msg.getPerformative()) + " message. Sender is "
	...
	private Method findServerMethod(Object cel, int performative) {
		...
		// NOTE: cachedMethods is transient --> If the agent has just moved it must be
		// recreated
		if (cachedMethods == null) {
			cachedMethods = new HashMap<>();
		}
		Method m = cachedMethods.get(key);
		if (m != null) {
			// Cache hit!
			return m;
		}
		// Note that we may have received a ContentElement that extends another one -->
		// Possibly there is no serving
		// method for the received ContentElement, but there is one for the parent
		// ContentElement
	...
	/**
	 * Makes this OntologyServer ignore all incoming messages belonging to a given
	 * conversation i.e. marked with the indicated conversation-id
	 */
	...
	/**
	 * Notifies this OntologyServer that a given conversation is finished and
	 * therefore it must no longer ignore messages belonging to it.
	 * 
	 * @see ignoreConversation(String)
	 */
