...
public class PlatformManagerImpl implements PlatformManager {
	...
	class ServiceEntry {
		...
		private Map<String, SliceEntry> slices;
	...
	PlatformManagerImpl(Profile p) throws ProfileException {
		...
		if (platformID == null || "".equals(platformID)) {
			try {
				...
				TransportAddress ta = tal.get(0);
				platformID = ta.getHost() + ":" + ta.getPort() + "/JADE";
			} catch (Exception e) {
	 ...
	 */
	public String addNode(NodeDescriptor dsc, List<ServiceDescriptor> nodeServices, boolean propagated) throws IMTPException, ServiceException, JADESecurityException {
	...
	private String localAddNode(NodeDescriptor dsc, List<ServiceDescriptor> nodeServices, boolean propagated) throws IMTPException, ServiceException, JADESecurityException {
		...
		if (nodeServices != null) {
			for (int i = 0; i < nodeServices.size(); ++i) {
				ServiceDescriptor service = (ServiceDescriptor) nodeServices.get(i);
	...
	private void broadcastAddNode(NodeDescriptor dsc, List<ServiceDescriptor> nodeServices) throws ServiceException {
	...
	private void localAddSlice(ServiceDescriptor serviceDsc, NodeDescriptor dsc, boolean propagated) throws IMTPException, ServiceException {
		...
		ServiceEntry e = services.get(serviceKey);
		if (e == null) {
	...
	private void localRemoveReplica(String address, boolean propagated) throws ServiceException {
		...
		try {
			...
			if (replicas.remove(key) == null) {
				...
				StringBuilder sb = new StringBuilder("Replica "+key.hashCode()+", "+key.getAddress()+"not found! Known replicas are: {");
	 ...
	 */
	public void addMainContainerNode(NodeDescriptor dsc, Vector<ServiceDescriptor> services) throws JADESecurityException {
		issueNewNodeCommand(dsc);
		for (int i = 0; i < services.size(); ++i) {
			ServiceDescriptor srvDsc = services.get(i);
	...
	public Vector<Slice> findAllSlices(String serviceKey) throws IMTPException, ServiceException {
		ServiceEntry e = services.get(serviceKey);
		if (e == null) {
			return null;
		} else {
	...
	private List getAllNodesInfo() {
		...
		Object[] allNodes = nodes.values().toArray();
		List<NodeInfo> infos = new ArrayList<>(allNodes.length);
		for (int i = 0; i < allNodes.length; ++i) {
		...
		Map<String, Vector> nodeServices = new HashMap<>();
		// Avoid concurrent modification exception
		...
		for (int j = 0; j < allServices.length; ++j) {
			...
			for (int i = 0; i < serviceNodes.length; i++) {
				...
				Vector v = nodeServices.get(nodeName);
				if (v == null) {
					v = new Vector<>();
	...
	private void monitor(NodeDescriptor dsc) {
		...
		if (parent != null) {
			...
			if (isLocalNode(parent)) {
				...
				NodeFailureMonitor failureMonitor = monitors.get(parent.getName());
				if (failureMonitor != null) {
	...
	private final class NodeInfo {
		...
		public void setServices(List<ServiceDescriptor> ss) {
			services = ss;
		}
