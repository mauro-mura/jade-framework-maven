...
/**
 * Implementation of <code><b>fipa.mts.mtp.iiop.std</b></code> specification for
 * delivering ACL messages over the OMG IIOP transport protocol.
 * 
...
public class MessageTransportProtocol implements MTP {
	...
	private static class MTSImpl extends FIPA._MTSImplBase {
		...
		public void message(FipaMessage aFipaMessage) {
			...
			// String tmp = "\n\n"+(new java.util.Date()).toString()+" RECEIVED IIOP
			// MESSAGE"+ "\n" + env.toString() + "\n" + new String(payload);
			// System.out.println(tmp);

			// MessageTransportProtocol.log(tmp); //Write in a log file for iiop incoming
			// message
	...
	public void deliver(String addr, Envelope env, byte[] payload) throws MTPException {
		try {
			...
			List<FIPA.AgentID> to = new ArrayList<>();
			while (itTo.hasNext()) {
				AID id = itTo.next();
			...
			Iterator<AID> itIntendedReceiver = env.getAllIntendedReceiver();
			List<FIPA.AgentID> intendedReceiver = new ArrayList<>();
			while (itIntendedReceiver.hasNext()) {
				AID id = itIntendedReceiver.next();
			...
			Iterator<Property> itUserDefProps = env.getAllProperties();
			List<FIPA.Property> userDefProps = new ArrayList<>();
			while (itUserDefProps.hasNext()) {
				Property p = itUserDefProps.next();
			...
			/*
			 * FIXME: Maybe several IDL Envelopes should be generated, one for every
			 * 'received' stamp... ReceivedObject[] received = env.getStamps();
			 * FIPA.ReceivedObject[] IDLreceived = new FIPA.ReceivedObject[received.length];
			 * for(int i = 0; i < received.length; i++) IDLreceived[i] =
			 * marshalReceivedObj(received[i]);
			 */
			...
			FIPA.Envelope IDLenv = new FIPA.Envelope(IDLto, IDLfrom, IDLcomments, IDLaclRepresentation,
					...
					IDLtransportBehaviour, IDLuserDefinedProperties);
			...
			// MessageTransportProtocol.log(tmp); // write in a log file for sent iiop
			// message
	...
	private FIPA.AgentID marshalAID(AID id) {
		...
		for (int i = 0; i < numOfResolvers; i++) {
			result.resolvers[i] = marshalAID(resolvers[i]); // Recursively marshal all resolvers, which are, in turn,
															// AIDs.
		}
	...
	private FIPA.DateTime marshalDateTime(Date d) {
		...
		// FIXME: This is truncated to the second
...
//Method to write on a file the iiop message log file
	/*
	 * public static synchronized void log(String str) { logFile.println(str);
	 * logFile.flush(); }
	 */
...
/**
 * This class represents an IIOP address. Three syntaxes are allowed for an IIOP
 * address (all case-insensitive): <code>
IIOPAddress ::= "ior:" (HexDigit HexDigit+)
              | "corbaname://" NSHost ":" NSPort "/" NSObjectID "#" objectName
              | "corbaloc:" HostName ":" portNumber "/" objectID
</code> Notice that, in the third case, BIG_ENDIAN is assumed by default. In the
 * first and second case, instead, the endianess information is contained within
 * the IOR definition.
 **/
class IIOPAddress implements TransportAddress {
	...
	private void initFromNS(String s) throws MTPException {
		...
		try {
			...
			List<NameComponent> name = new ArrayList<>();
	...
	private abstract static class CDRCodec {
		...
		protected CDRCodec(String hexString) {
			...
			readIndex = 1;
			writeBuffer = new StringBuilder(255);
		}
