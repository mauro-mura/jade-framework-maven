...
public class HTTPServer extends Thread {
	...
	public HTTPServer(String interfaceAddress, int p, InChannel.Dispatcher d, int m, String s, int t,
			boolean changePortIfBusy) throws IOException {
		...
		try {
			...
			// #PJAVA_EXCLUDE_END
			/*
			 * #PJAVA_INCLUDE_BEGIN server = new ServerSocket(port); #PJAVA_INCLUDE_END
			 */
	...
	public static class ServerThread extends Thread {
		...
		public void run() {
			try {
				...
				// #DOTNET_EXCLUDE_END
				/*
				 * #DOTNET_INCLUDE_BEGIN codec = new XMLCodec(); #DOTNET_INCLUDE_END
				 */
				...
				do {
					...
					if (HTTPIO.OK.equals(responseMsg)) {
						...
						// #DOTNET_EXCLUDE_END
						/*
						 * #DOTNET_INCLUDE_BEGIN System.IO.StringReader sr = new System.IO.StringReader(
						 * envelope.toString() ); #DOTNET_INCLUDE_END
						 */
						Envelope env = codec.parse(sr);

						/*
						 * #DOTNET_INCLUDE_BEGIN //There are problems if PayloadEncoding is set to
						 * US-ASCII if (env.getPayloadEncoding() == null)
						 * env.setPayloadEncoding(XMLCodec.CHARS_CODEC); #DOTNET_INCLUDE_END
						 */
