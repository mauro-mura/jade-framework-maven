...
public class MessageTransportProtocol implements MTP {
	 ...
	 *
	 * Customizable parameters read from profile:
	 * <UL>
	 * <LI><B>port</B>: the port this HTTP server listens to.</LI>
	 * <LI><B>numKeepAlive</B>: Maximum number of keep-alive connections. Default
	 * value is 10. Set to 0 to disable keep-alive coonections (possible performance
	 * impact).</LI>
	 * <LI><B>proxyHost</B>: Proxy host name or IP-address. No default value.</LI>
	 * <LI><B>proxyPort</B>: Default value is 1080.</LI>
	 * <LI><B>outPort</B>: Fix port to be used by HTTP client (for firewall
	 * configuration). Default value is freely chosen by Java</LI>
	 * <LI><B>parser</B>: XML SAX2 parser implementation to use. Default value is
	 * JDK 1.4 default parser.</LI>
	 * <LI><B>policy</B>: "conservative" (default value) or "aggressive". (see
	 * documentation for details).</LI>
	 * <LI><B>timeout</B>: Timeout for keep-alive connections. Default value is 1
	 * min. 0 means infinite.</LI>
	 * </UL>
	...
	private TransportAddress activateServer(InChannel.Dispatcher disp, TransportAddress ta, Profile p)
		...
		try {
			if (ta != null) {
				...
				try {
					...
					// #DOTNET_EXCLUDE_END
					/*
					 * #DOTNET_INCLUDE_BEGIN hta = new
					 * HTTPAddress(InetAddress.getLocalHost().getHostName(),port, false);
					 * #DOTNET_INCLUDE_END
					 */
		...
		try {
			...
			if (actualPort != port) {
				// The selected port is busy and a new one was selected --> Update the transport
				// address
				...
				/*
				 * #DOTNET_INCLUDE_BEGIN hta = new
				 * HTTPAddress(InetAddress.getLocalHost().getHostName(),actualPort, useHttps);
				 * #DOTNET_INCLUDE_END
				 */
			}
	...
	public void deliver(String addr, Envelope env, byte[] payload) throws MTPException {
		...
		try {
			...
			// Request body
			// String body = HTTPIO.createHTTPBody(env,boundary.toString(),new
			// String(payload));
			...
			// HTTP header
			// StringBuffer req =
			// HTTPIO.createHTTPHeader(host,body.length(),connPol,boundary.toString(),useProxy);
